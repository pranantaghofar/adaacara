<?php
    ...
    $pageOgImage = (string) ($page['og_image'] ?? '');
...
<head>
    ...
    <style>
        ...
        .aa-photobooth-entry-toast {
            ...
            background: #fff7df;
            color: #5f3f06;
            box-shadow: 0 12px 28px rgba(146, 64, 14, .12);
            font-size: 12px;
            font-weight: 800;
            line-height: 1.35;
        }

        .aa-photobooth-entry-toast.is-visible {
            display: flex;
        }

        .aa-photobooth-entry-toast-close {
            display: inline-flex;
            align-items: center;
        ...
        .aa-photobooth-entry-toast-close:hover {
            background: #ffffff;
            border-color: rgba(107, 33, 168, .28);
            color: #4c1d95;
        }

        body.aa-editor-tablet-mode .aa-photobooth-entry-toast,
        body.aa-editor-mobile-mode .aa-photobooth-entry-toast {
            top: 88px;
...
<body class="aa-app-ui<?= $isBusinessProfileIntentProject ? ' aa-business-profile-editor' : '' ?>">
    <div id="aaDesktopOnlyModal" class="aa-desktop-only-modal" role="dialog" aria-modal="true"
        ...
        <div class="aa-desktop-only-card">
            ...
            <h2 id="aaDesktopOnlyTitle">Buka lewat tablet, laptop, atau PC dulu ya</h2>
            <p>Silakan buka AdaAcara Design Studio melalui tablet, laptop, atau PC agar canvas, drag, resize, dan publish
                berjalan stabil. Dukungan mobile kecil sedang dalam pengembangan.</p>
        </div>
    </div>

    <div id="aaTabletLandscapeModal" class="aa-desktop-only-modal aa-tablet-landscape-modal" role="dialog" aria-modal="true"
        aria-labelledby="aaTabletLandscapeTitle">
        <div class="aa-desktop-only-card">
            <div class="aa-desktop-only-icon"><i class="fa fa-tablet-screen-button"></i></div>
            <h2 id="aaTabletLandscapeTitle">Putar tablet ke mode landscape</h2>
            <p>Editor aktif di tablet saat posisi layar landscape agar canvas, toolbar, dan panel pengaturan tetap nyaman digunakan.</p>
        </div>
    </div>
    ...
    <div class="aa-studio-shell">
        <header class="aa-topbar">
            <div class="aa-topbar-brand aa-editor-brand-card min-w-0">
                <div class="aa-editor-brand-logo" aria-label="AdaAcara Design Studio">
                    <strong>ADAACARA</strong>
                    <span>DESIGN STUDIO</span>
                </div>
                <span class="aa-editor-brand-divider" aria-hidden="true"></span>
                <div class="aa-editor-brand-project min-w-0">
                    <h1 class="m-0 truncate">
                        <?= esc($pageTitle) ?>
                        <span class="aa-editor-brand-live-dot" aria-hidden="true"></span>
                    </h1>
                    <?php if ($isBusinessProfileIntentProject): ?>
                    <span class="aa-editor-brand-type-pill">Business Profile</span>
                    <?php endif ?>
                    <p id="aaBrandSaveState" class="m-0 truncate">Disimpan otomatis</p>
                </div>
            </div>
            <button id="aaMobileToolsBtn" class="aa-action-btn aa-mobile-tools-btn" type="button"
                aria-label="Buka menu editor" aria-expanded="false">
            ...
            <div class="aa-topbar-controls flex min-w-0 flex-1 items-center justify-center gap-2">
                <div class="aa-topbar-group aa-topbar-history" aria-label="Riwayat perubahan">
                    <button id="aaUndoBtn" class="aa-action-btn" type="button"><i
                            class="fa fa-rotate-left"></i><span class="aa-topbar-action-text">Undo</span></button>
                    <button id="aaRedoBtn" class="aa-action-btn" type="button"><i
                            class="fa fa-rotate-right"></i><span class="aa-topbar-action-text">Redo</span></button>
                </div>
                <button id="aaMobileShareBtn" class="aa-action-btn aa-mobile-share-btn" type="button"
                    aria-label="Buka aksi share" aria-expanded="false">
                    Share
                </button>
                <div class="aa-topbar-size-controls" aria-label="Resize halaman" data-aa-resize-dropdown>
	                ...
	                <button id="aaMobileSaveBtn" class="aa-action-btn aa-mobile-save-btn" type="button"
        ...
        <?php $showPhotoboothEntryToast = ! empty($hasPhotoboothProjectSurface) && empty($canUseGuestMemories); ?>
        <div id="aaPhotoboothEntryToast"
            class="aa-photobooth-entry-toast<?= $showPhotoboothEntryToast ? ' is-visible' : '' ?>"
            role="status" aria-live="polite">
            <span>Photobooth belum aktif di akun ini. Silakan berlangganan Plus terlebih dahulu.</span>
            <button id="aaPhotoboothEntryToastClose" class="aa-photobooth-entry-toast-close"
                type="button" aria-label="Tutup">⛌</button>
        </div>

        <nav class="aa-tablet-action-dock" aria-label="Aksi editor tablet">
            <button class="aa-action-btn" type="button" data-aa-tablet-action="preview">Preview</button>
            <button class="aa-action-btn aa-primary" type="button" data-aa-tablet-action="save">Save Draft</button>
            <?php if (! empty($canSaveTemplate)): ?>
            <button class="aa-action-btn" type="button" data-aa-tablet-action="save-template">Save as Template</button>
            <?php endif ?>
            <?php if ($showPublishButton): ?>
            <button class="aa-action-btn aa-publish" type="button" data-aa-tablet-action="publish"
                title="<?= esc($publishButtonTitle, 'attr') ?>">Publish</button>
            <?php endif ?>
        </nav>
        ...
        <main class="aa-workspace">
            ...
            <section id="aaStageWrap" class="aa-stage-wrap">
                <div id="aaStageViewport" class="aa-stage-viewport">
                    <div id="aaStage" class="aa-stage is-page-loading">
                        <div class="aa-editor-mode-strip">
                            <div class="aa-editor-mode-toggle" aria-label="Mode editor">
                                ...
                                <?php endif ?>
                                <button id="aaEditOpeningBtn" class="aa-editor-mode-btn" type="button">COVER</button>
            ...
            <aside class="aa-rightbar p-4">
                <div class="aa-mobile-properties-bar" aria-label="Mobile text properties">
                    <span><i class="fa fa-keyboard" aria-hidden="true"></i> Edit teks</span>
                    <div>
                        <button id="aaMobileTextDoneBtn" class="aa-mobile-properties-action is-primary" type="button">Simpan</button>
                        <button id="aaMobileTextCloseBtn" class="aa-mobile-properties-action" type="button">Tutup</button>
                    </div>
                </div>
                ...
                <?php $premiumCardActive = ! empty($hasActiveMembership); ?>
                <?php $showPhotoboothUpgradeCard = $isPhotoboothIntentProject && empty($canUseGuestMemories); ?>
                <section class="aa-premium-upgrade-card <?= $premiumCardActive ? 'is-active' : '' ?>"
                    aria-label="<?= $premiumCardActive ? 'Paket aktif' : ($showPhotoboothUpgradeCard ? 'Aktifkan Photobooth' : 'Upgrade ke Premium') ?>">
                    <div class="aa-premium-upgrade-card-title">
                        <i class="fa <?= $showPhotoboothUpgradeCard && ! $premiumCardActive ? 'fa-camera' : 'fa-crown' ?>" aria-hidden="true"></i>
                        <strong><?= $premiumCardActive ? 'Paket aktif' : ($showPhotoboothUpgradeCard ? 'Aktifkan Photobooth' : 'Upgrade ke Premium') ?></strong>
                    </div>
                    <ul>
                        <?php if ($showPhotoboothUpgradeCard && ! $premiumCardActive): ?>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span>Buat frame Photobooth</span></li>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span>Buka halaman Memories</span></li>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span>QR Photobooth & galeri tamu</span></li>
                        <?php else: ?>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span><?= $premiumCardActive ? 'Fitur premium aktif' : 'Buka semua fitur' ?></span></li>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span>Publish link tanpa batas</span></li>
                            <li><i class="fa fa-check" aria-hidden="true"></i><span>Akses template premium</span></li>
                        <?php endif; ?>
                    </ul>
                    <?php if ($premiumCardActive): ?>
                        <span class="aa-premium-upgrade-card-button is-active">
                            <i class="fa fa-circle-check" aria-hidden="true"></i>
                            <span>Paket Aktif</span>
                        </span>
                    <?php else: ?>
                        <a class="aa-premium-upgrade-card-button" href="<?= esc($plansUrl, 'attr') ?>" target="_blank"
                            rel="noopener">
                            <i class="fa fa-rocket" aria-hidden="true"></i>
                            <span><?= $showPhotoboothUpgradeCard ? 'Lihat Paket Plus' : 'Upgrade Sekarang' ?></span>
                        </a>
                    <?php endif; ?>
                </section>
    ...
    (function() {
        ...
        function setStatus(message, tone) {
            ...
            if (els.aaBrandSaveState) {
                els.aaBrandSaveState.textContent = message;
                els.aaBrandSaveState.classList.toggle('is-error', tone === 'error');
                els.aaBrandSaveState.classList.toggle('is-saving', tone === 'saving');
            }
        ...
        function bindPhotoboothEntryToast() {
            els.aaPhotoboothEntryToastClose?.addEventListener('click', () => {
                els.aaPhotoboothEntryToast?.classList.remove('is-visible');
            });
        }

        function hidePendingEditorToast() {
            if (els.aaEditorToast?.classList.contains('is-saving')) {
                hideEditorToast();
            }
        }
        ...
        function showCanvasLoading(message = 'Memuat desain...') {
            ...
            els.aaStage?.classList.add('is-page-loading');
        ...
        function hideCanvasLoading() {
            window.requestAnimationFrame(() => {
                window.requestAnimationFrame(() => {
                    ...
                    els.aaStage?.classList.remove('is-page-loading');
                    if (typeof syncZoomViewport === 'function') syncZoomViewport();
                });
            });
        }
        ...
        function normalizeSlug(value) {
	        ...
	        function clampEditorZoom(value) {
	            ...
	            return Math.max(0.15, Math.min(1.5, next));
	        }

	        function getZoomFocusTarget(options = {}) {
	            if (options.focusActiveObject === false || !state.canvas) return null;
	            if (state.isCropping === true || state.cropBox || state.cropTarget) return null;
	            const active = state.canvas.getActiveObject?.();
	            if (!active || active.customType === 'background') return null;
	            if (active.customType === 'crop-helper' || active === state.cropBox) return null;
	            if (active.visible === false) return null;
	            return active;
	        }

	        function focusZoomOnObject(target) {
	            if (!target || !els.aaStageWrap || !state.canvas?.upperCanvasEl) return;
	            requestAnimationFrame(() => {
	                const stillActive = state.canvas?.getActiveObject?.() === target;
	                const stillOnCanvas = state.canvas?.getObjects?.().includes(target);
	                if (!target || (!stillActive && !stillOnCanvas)) return;
	                if (target.visible === false) return;
	                const wrapRect = els.aaStageWrap.getBoundingClientRect();
	                const canvasRect = state.canvas.upperCanvasEl.getBoundingClientRect();
	                const objectRect = target.getBoundingRect?.(true, true);
	                if (!wrapRect || !canvasRect || !objectRect) return;
	                ...
	                const objectCenterX = canvasRect.left + (objectRect.left + objectRect.width / 2) * scaleX;
	                const objectCenterY = canvasRect.top + (objectRect.top + objectRect.height / 2) * scaleY;
	                const viewportCenterX = wrapRect.left + els.aaStageWrap.clientWidth / 2;
	                const viewportCenterY = wrapRect.top + els.aaStageWrap.clientHeight / 2;
	                const nextLeft = els.aaStageWrap.scrollLeft + objectCenterX - viewportCenterX;
	                const nextTop = els.aaStageWrap.scrollTop + objectCenterY - viewportCenterY;
	                els.aaStageWrap.scrollTo({
	        ...
	        function setEditorZoom(value, options = {}) {
	            const nextZoom = clampEditorZoom(value);
	            if (Math.abs(nextZoom - (Number(state.zoom) || 1)) < 0.001) return;
	            const zoomFocusTarget = getZoomFocusTarget(options);
	            state.zoom = nextZoom;
	            ...
	            updateZoom();
	            if (zoomFocusTarget) {
	                focusZoomOnObject(zoomFocusTarget);
	            }
	        }
	        ...
	        function fitZoomUnlessManual() {
	            if (state.zoomManuallyAdjusted === true) {
	                updateZoom();
	                return;
	            }
	            fitZoom();
        ...
        function sanitizeFabricObject(object) {
            ...
            if (object.aaMaterialPreset) {
                object.fill = object.aaMaterialFallback || object.fill || '#111827';
            }
        ...
        function refreshImageBorderRadius(canvas) {
	            canvas.getObjects().forEach(object => {
	                if (object.type === 'image') {
	                    if (object.aaImageEffectPreset && Array.isArray(object.filters) && object.filters.length) {
	                        object.set('filters', []);
	                    }
	                    ...
	                    object.getObjects().forEach(child => {
	                        if (child.type === 'image') {
                            if (child.aaImageEffectPreset && Array.isArray(child.filters) && child.filters.length) {
                                child.set('filters', []);
                            }
...
function aaWithStableSelectionForHistory(callback) {
       ...
       function snapshot() {
            ...
            try {
                json = aaWithStableSelectionForHistory(function () {
                    ...
                    data.editMode = state.editMode === 'opening' ? 'opening' : (state.editMode === 'photobooth' ? 'photobooth' : 'pages');
    ...
    const restoreMode = options.mode === 'opening' || options.mode === 'pages' || options.mode === 'photobooth'
        ? options.mode
        : (data.editMode === 'opening' ? 'opening' : (data.editMode === 'photobooth' ? 'photobooth' : 'pages'));
    ...
    if (restoreMode === 'photobooth') {
        const photoboothFrames = normalizePhotoboothFrames(data.photoboothFrames || state.photoboothFrames);
        const activePhotoboothFrameIndex = Math.max(0, Math.min(data.activePhotoboothFrameIndex || 0, photoboothFrames.length - 1));

        return {
            data,
            mode: 'photobooth',
            pageData: photoboothFrames[activePhotoboothFrameIndex] || createPhotoboothFrameData(1),
            photoboothFrames,
            activePhotoboothFrameIndex
        };
    }
...
function aaApplyPageDataToCanvasAtomic(pageData, done) {
    ...
    fabric.util.enlivenObjects(targetObjectsJson, function (objects) {
       ...
       function restoreFromSnapshot(json, options = {}) {
            ...
            } else if (parsed.mode === 'photobooth') {
                state.photoboothFrames = parsed.photoboothFrames;
                state.activePhotoboothFrameIndex = parsed.activePhotoboothFrameIndex;
                renderPageList?.();
            } else {
        ...
        function undo() {
            ...
            const mode = state.editMode === 'opening' ? 'opening' : (state.editMode === 'photobooth' ? 'photobooth' : 'pages');
        ...
        function renderCountdownDatePicker(baseDate = null) {
	        ...
	        function positionCountdownDatePicker() {
	            ...
	            const isMobileCountdownPicker = document.body.classList.contains('aa-editor-mobile-mode') ||
	                window.matchMedia('(max-width: 767px)').matches;
	            const toolbarRect = isMobileCountdownPicker && els.aaCountdownContextToolbar?.classList.contains('is-visible') ?
	                els.aaCountdownContextToolbar.getBoundingClientRect() : null;
	            const visibleBottom = toolbarRect ?
	                Math.min(viewportTop + viewportHeight, toolbarRect.top - 10) :
	                viewportTop + viewportHeight;
	            const left = Math.max(viewportLeft + margin, Math.min(inputRect.left, viewportLeft + viewportWidth -
	                pickerRect.width - margin));
	            let top = inputRect.bottom + 8;
	            if (isMobileCountdownPicker && top + pickerRect.height > visibleBottom - margin) {
	                top = inputRect.top - pickerRect.height - 8;
	            }
	            top = Math.max(viewportTop + margin, Math.min(top, visibleBottom - pickerRect.height - margin));
        ...
        function isGifSource(src = '', mime = '') {
            const source = String(src || '').split('?')[0].toLowerCase();
            return String(mime || '').toLowerCase() === 'image/gif' || source.endsWith('.gif');
        }

        function gifMetadataForSource(src = '', mime = '') {
            if (!isGifSource(src, mime)) return {};
            return {
                aaMediaKind: 'gif',
                aaAnimatedSrc: src,
            };
        ...
        function editorAssetDisplayImageSrc(asset = {}) {
            const src = String(asset?.src || '');
            const mime = asset?.mime || asset?.mimeType || '';
            const thumbnail = String(asset?.thumbnail || '');
            if (isGifSource(src, mime) && thumbnail && !isGifSource(thumbnail, '')) {
                return thumbnail;
            }
            return src;
        }
        ...
        function applySocialLinkValue(values = {}) {
            const active = typeof getInteractionApplyTarget === 'function'
                ? getInteractionApplyTarget(isSocialLinkInteractionObject)
        ...
        function aaPreserveImageAspectRatioOnScale(event) {
            ...
            const transformAction = String(event?.transform?.action || state.canvas?._currentTransform?.action || '');
            if (aaIsTextSideCorner(corner) && transformAction.indexOf('crop-') === 0) {
                return false;
            }
        ...
        function aaResolveAccidentalImageActiveSelection(event = null) {
            ...
            const sortedObjects = objects
                .slice()
                .sort((a, b) => stack.indexOf(b) - stack.indexOf(a));
            const topMost = sortedObjects.find(object => !aaIsLargeImageObject(object)) || sortedObjects[0];
        ...
        function aaObjectContainsCanvasPoint(object, point) {
            ...
            try {
                if (!object.containsPoint(point)) return false;
                return !aaIsTransparentCanvasTargetAtPoint(object, point);
            } catch (error) {
                return false;
            }
        }

        function aaIsTransparentCanvasTargetAtPoint(object, point) {
            if (!state.canvas || !object || !point || object.perPixelTargetFind !== true) return false;
            if (typeof state.canvas.isTargetTransparent !== 'function') return false;
            try {
                return state.canvas.isTargetTransparent(object, point.x, point.y) === true;
            } catch (error) {
                return false;
            }
        }
        ...
        function resolveLargeImageClickSelection(event = null) {
	        ...
	        function createFramePlaceholderShape(shape, width, height) {
	            const shapeName = String(shape || 'rounded');
	            const frameWidth = Math.max(80, Number(width) || 320);
	            const frameHeight = Math.max(80, Number(height) || 240);
	            let object = null;
	            ...
	            if (shapeName === 'rounded') {
	                object = new fabric.Rect({
	                    width: frameWidth,
	                    height: frameHeight,
	                    rx: Math.max(18, Math.min(frameWidth, frameHeight) * 0.12),
	                    ry: Math.max(18, Math.min(frameWidth, frameHeight) * 0.12),
	                    originX: 'center',
	                    originY: 'center',
	                });
	            } else if (typeof createImageFrameClipPath === 'function') {
	                object = createImageFrameClipPath({
	                    width: frameWidth,
	                    height: frameHeight,
	                }, shapeName);
	            }
	        ...
	        function createFramePlaceholderGroup(shapeName = 'rounded', width = 320, height = 240) {
	            const shape = String(shapeName || 'rounded');
	            const frameShape = createFramePlaceholderShape(shape, width, height);
	            ...
	            const label = new fabric.Text('Tambahkan foto', {
	                left: 0,
	                top: 42,
	                originX: 'center',
	                originY: 'center',
	            ...
	            const group = new fabric.Group([frameShape, plus, label], {
	                customType: 'frame-placeholder',
	                name: 'Image Frame',
	                aaFramePlaceholder: true,
	                aaFramePlaceholderLabel: 'Tambahkan foto',
	        ...
	        function addImageFramePlaceholder(shapeName = 'rounded') {
	            if (!state.canvas || state.isCropping) return;
	            const shape = String(shapeName || 'rounded');
	            const width = 320;
	            const height = shape === 'circle' ? 320 : 240;
	            const group = createFramePlaceholderGroup(shape, width, height);
	            centerObject(group);
	            syncImageFrameButtons?.(group);
	            setStatus('Frame kosong ditambahkan. Isi dengan upload atau Media Library.');
	        }

	        function isFramePlaceholderObject(object) {
	            return Boolean(object && object.customType === 'frame-placeholder');
	        }

	        function updateFramePlaceholderShape(target, shapeName = 'rounded') {
	            if (!state.canvas || !isFramePlaceholderObject(target)) return false;
	            const shape = String(shapeName || 'rounded');
	            const center = typeof target.getCenterPoint === 'function'
	                ? target.getCenterPoint()
	                : new fabric.Point(Number(target.left) || 0, Number(target.top) || 0);
	            const width = Math.max(80, Number(target.width) || (shape === 'circle' ? 320 : 240));
	            const height = Math.max(80, Number(target.height) || (shape === 'circle' ? width : 240));
	            const replacement = createFramePlaceholderGroup(shape, width, shape === 'circle' ? width : height);
	            const index = Math.max(0, state.canvas.getObjects().indexOf(target));
	            replacement.set({
	                left: center.x,
	                top: center.y,
	            ...
	            replacement.setCoords();
	            state.__aaFramePlaceholderFillTarget = replacement;
	            state.__aaMobileInteractionDrawerTarget = replacement;
	            state.canvas.requestRenderAll();
	            syncImageFrameButtons?.(replacement);
	            syncInteractionPopover?.(replacement);
	            snapshot();
	            setStatus('Bentuk frame diperbarui');
	            return true;
	        }
        ...
        function copyImageReplaceStyle(source, target, options = {}) {
            ...
            if (source.aaImageEffectPreset) {
                target.set('filters', []);
            } else if (Array.isArray(source.filters) && source.filters.length) {
        ...
        function resetImageOutlineMetadataForReplacement(image, src, options = {}) {
            if (!image) return;
            const cleanSrc = String(src || readImageSourceUrl(image) || '').trim();
            const isRemoveBgSource = isRemoveBgAssetSource(cleanSrc);
            image.set({
                aaRemovedBg: isRemoveBgSource,
                aaRemovedBgSrc: isRemoveBgSource ? cleanSrc : '',
                aaImageOutlineBaseSrc: cleanSrc,
                aaImageOutlineWidth: 0,
                aaImageOutlineDraftWidth: 0,
                aaImageOutlineAppliedSrc: '',
                aaImageAlphaOutline: false,
                aaOriginalImageSrc: cleanSrc,
                aaOriginalImageName: options.aaOriginalImageName || options.galleryImageName || image.galleryImageName || '',
            });
            delete image.aaImageOutlineAlphaEligible;
            delete image.__aaImageOutlineAlphaPromise;
            delete image.__aaImageOutlineAlphaToken;
            image.dirty = true;
        }
        ...
        function replaceImageObject(source, replacement, options = {}) {
            ...
            if (options.resetOutlineMetadata === true) {
                resetImageOutlineMetadataForReplacement(replacement, options.replacementImageSrc || options.aaOriginalImageSrc || options.galleryImageSrc || readImageSourceUrl(replacement), options);
            }
        ...
        function aaImageHasVisibleCropForOutline(image, sourceImage = null) {
            if (!image || image.type !== 'image') return false;
            const natural = sourceImage ? {
            ...
            const cropX = Math.max(0, Number(image.cropX) || 0);
            const cropY = Math.max(0, Number(image.cropY) || 0);
            const width = Math.max(1, Number(image.width) || natural.width || 1);
            const height = Math.max(1, Number(image.height) || natural.height || 1);
            const epsilon = 1;

            return cropX > epsilon ||
                cropY > epsilon ||
                width < natural.width - cropX - epsilon ||
                height < natural.height - cropY - epsilon;
        }

        function aaCreateCroppedOutlineBaseSource(image, sourceImage) {
            ...
            const cropX = Math.max(0, Math.min(Math.round(Number(image.cropX) || 0), naturalWidth - 1));
            const cropY = Math.max(0, Math.min(Math.round(Number(image.cropY) || 0), naturalHeight - 1));
            const cropWidth = Math.max(1, Math.min(Math.round(Number(image.width) || naturalWidth), naturalWidth - cropX));
            const cropHeight = Math.max(1, Math.min(Math.round(Number(image.height) || naturalHeight), naturalHeight - cropY));

            if (cropWidth * cropHeight > 16000000) {
                throw new Error('Area crop terlalu besar untuk outline. Perkecil gambar lalu coba lagi.');
            }
            ...
            if (!ctx) {
                throw new Error('Browser belum bisa memproses crop outline gambar ini.');
            }
        ...
        async function aaResolveOutlineProcessingSource(object, baseSrc) {
            const image = findImageForOutline(object);
            const sourceImage = await loadImageElementForProcessing(baseSrc);
            if (!image || image.type !== 'image' || image !== object || !aaImageHasVisibleCropForOutline(image, sourceImage)) {
                return {
                    baseSrc,
                    sourceImage,
                    fromVisibleCrop: false,
                };
            }

            const croppedBaseSrc = aaCreateCroppedOutlineBaseSource(image, sourceImage);
            const croppedSourceImage = await loadImageElementForProcessing(croppedBaseSrc);
            return {
                baseSrc: croppedBaseSrc,
                sourceImage: croppedSourceImage,
                fromVisibleCrop: true,
            };
        }
        ...
        async function applyImageAlphaOutline(options = {}) {
            ...
            try {
                const outlineSource = await aaResolveOutlineProcessingSource(active, baseSrc);
                const sourceImage = outlineSource.sourceImage;
                const processingBaseSrc = outlineSource.baseSrc;
                const outlinedSrc = width > 0 ? createAlphaOutlineImageDataUrl(sourceImage, color, width) : processingBaseSrc;
                ...
                const replaced = replaceImageObject(active, replacement, {
                    ...
                    aaRemovedBg: active.aaRemovedBg === true || baseIsRemoveBg || outlineSource.fromVisibleCrop === true,
                    aaRemovedBgSrc: baseIsRemoveBg ? (active.aaRemovedBgSrc || baseSrc) : (outlineSource.fromVisibleCrop ? processingBaseSrc : ''),
                    aaImageOutlineBaseSrc: processingBaseSrc,
                    ...
                    aaOriginalImageSrc: processingBaseSrc,
                ...
                if ((keepPopoverOpen || state.keepImageOutlineDrawerOpen === true) && isImageOutlineTarget(nextActive)) {
        ...
        async function createMagicLayerTextObjects(blueprint, geometry) {
            ...
            for (const block of blueprint.blocks.slice(0, 24)) {
                ...
                const textbox = new fabric.Textbox(block.text, {
                    ...
                    angle: region.angle,
                    fontFamily,
                    fontSize: Math.max(10, Math.min(180, block.fontSize > 0 ? block.fontSize * Math.max(region.height / Math.max(1, block.height), 0.2) : region.height * (fontFamily === 'Great Vibes' ? 0.95 : 0.82))),
                    fill: block.color,
                    textAlign: block.align,
                    fontWeight: block.weightHint || '600',
                    fontStyle: block.italic ? 'italic' : 'normal',
                    lineHeight: 1.12,
                    customType: 'text',
                    name: 'Magic Layer Text',
                    aaSource: 'magic-layer-ai',
        ...
        function placeImageAtPointer(image, pointer) {
	        ...
	        function isImageReplaceTarget(object) {
	            return Boolean(object && (object.type === 'image' || object.customType === 'photo-frame' ||
	                isFramePlaceholderObject(object)));
	        }

	        function getFramePlaceholderTarget() {
	            const active = state.canvas?.getActiveObject?.();
	            if (isFramePlaceholderObject(active)) return active;
	            const stored = state.__aaFramePlaceholderFillTarget || null;
	            if (stored && isFramePlaceholderObject(stored) && state.canvas?.getObjects?.().includes(stored)) {
		        ...
		        function beginFramePlaceholderFillMode() {
		            if (guardMediaLibraryFeature()) return false;
		            const target = getFramePlaceholderTarget();
	            if (!target) {
	                setStatus('Pilih frame kosong terlebih dahulu.', 'error');
	                return false;
	            }
	            if (target.locked === true) {
	                setStatus('Frame terkunci. Unlock dulu untuk mengisi foto.', 'error');
	                return false;
	            }
		            state.__aaFramePlaceholderFillTarget = target;
		            state.canvas.setActiveObject(target);
		            state.mediaMode = 'frame-placeholder-fill';
		            state.canvas.requestRenderAll();
		            hidePendingEditorToast();
		            setStatus('Pilih foto untuk mengisi frame.');
		            return true;
		        }

	        function fillFramePlaceholderWithImage(target, image, options = {}) {
	            if (!state.canvas || !isFramePlaceholderObject(target) || !image) return false;
	            if (target.locked === true && options.allowLockedReplace !== true) {
	                setStatus('Frame terkunci. Unlock dulu untuk mengisi foto.', 'error');
	                return false;
	            }

	            const shape = String(target.aaImageFrameShape || 'rounded');
	            const displayedWidth = Math.max(1, target.getScaledWidth ? target.getScaledWidth() : (target.width || 320));
	            const displayedHeight = Math.max(1, target.getScaledHeight ? target.getScaledHeight() : (target.height || 240));
	            const cover = applyImageCoverToFrame(image, displayedWidth, displayedHeight);
	            const targetIndex = Math.max(0, state.canvas.getObjects().indexOf(target));
	            ...
	            image.set({
	                ...
	                scaleX: displayedWidth / Math.max(1, cover.cropWidth),
	                scaleY: displayedHeight / Math.max(1, cover.cropHeight),
	                flipX: target.flipX === true,
	                flipY: target.flipY === true,
	                opacity: Number.isFinite(Number(target.opacity)) ? target.opacity : 1,
	                customType: 'image',
	                aaImageFrameShape: shape,
	                aaOriginalImageSrc: options.aaOriginalImageSrc || options.replacementImageSrc || readImageSourceUrl(image),
	                aaOriginalImageName: options.aaOriginalImageName || options.galleryImageName || '',
	                borderRadius: shape === 'rounded' ? Math.max(24, Math.min(displayedWidth, displayedHeight) * 0.12) : 0,
	                objectCaching: false,
	            });
	            applyImageBorderRadius(image, image.borderRadius || 0);
	            if (shape !== 'none' && shape !== 'rounded' && typeof createImageFrameClipPath === 'function') {
	                image.set('clipPath', createImageFrameClipPath(image, shape));
	            }

	            state.canvas.remove(target);
	            state.canvas.insertAt(image, targetIndex, false);
        ...
        function syncMediaDropPreview(event) {
            ...
            if (!target || !setMediaDropPreviewRect(target)) {
	            ...
	            const isFrameFillTarget = isFramePlaceholderObject(target);
	            const premiumReplaceLocked = !locked && !isFrameFillTarget && !canUsePremiumFeature();
	            if (els.aaMediaDropPreviewLabel) {
	                els.aaMediaDropPreviewLabel.textContent = locked
	                    ? 'Gambar terkunci'
	                    : (isFrameFillTarget ? 'Lepas untuk isi frame' : (premiumReplaceLocked ? 'Replace gambar untuk member' : 'Lepas untuk ganti gambar'));
	            }
        ...
        function handleMediaDrop(event) {
            ...
            event.stopPropagation();
	            ...
	            const target = findImageAtPointer(pointer);
	            if (target) {
	                if (isFramePlaceholderObject(target)) {
	                    if (target.locked === true) {
	                        setStatus('Frame terkunci. Unlock dulu untuk mengisi foto.', 'error');
	                        return;
	                    }
	                    insertImage(asset.src, {
	                        ...
	                        aaOriginalImageName: asset.name || 'Frame photo',
	                    });
	                    setStatus('Foto masuk ke frame');
	                    return;
	                }
        ...
        function insertImage(url, options = {}) {
            fabric.Image.fromURL(url, function(image) {
                ...
                image.set({
                    ...
                    aaOriginalImageName: options.aaOriginalImageName || options.galleryImageName || '',
                    ...gifMetadataForSource(options.aaAnimatedSrc || url, options.mime || ''),
                    borderRadius: 0,
                    objectCaching: false,
                });
                resetImageCropState(image);
	                ...
	                const replaceTarget = options.replaceTarget || state.canvas.getActiveObject();
		                if ((state.mediaMode === 'replace' || options.replaceTarget) && isImageReplaceTarget(replaceTarget)) {
		                    if (isFramePlaceholderObject(replaceTarget)) {
		                        fillFramePlaceholderWithImage(replaceTarget, image, {
		                            ...options,
		                            replacementImageSrc: options.aaOriginalImageSrc || url,
		                        });
		                        if (typeof options.onReplaceComplete === 'function') {
		                            options.onReplaceComplete(image);
		                        }
        ...
        function setBackgroundImageStatus(message = '', type = '') {
            ...
            const surfaceLabel = state.editMode === 'photobooth' ? 'frame Photobooth' : 'halaman';
            els.aaBackgroundImageStatus.textContent = message ||
                `Maksimal 1MB. Gambar akan auto-cover mengikuti ukuran ${surfaceLabel}.`;
        ...
        function syncBackgroundImageControls() {
            ...
            if (els.aaBackgroundInput && state.canvas) {
                els.aaBackgroundInput.value = normalizeColor(state.canvas.backgroundColor || '#ffffff');
            }
        ...
        function setCanvasBackgroundImage(src, options = {}) {
            ...
            fabric.Image.fromURL(src, image => {
                ...
                const gifMeta = isGifSource(options.aaAnimatedSrc || src, options.mime || '') ? {
                    aaMediaKind: 'gif',
                    aaAnimatedSrc: String(options.aaAnimatedSrc || src).startsWith('data:') ? '' : (options.aaAnimatedSrc || src),
                } : {};
                image.set({
                    src,
                    crossOrigin: 'anonymous',
                ...
                state.canvas.requestRenderAll();
                snapshot();
                setBackgroundImageStatus(state.editMode === 'photobooth' ? 'Background frame Photobooth terpasang.' : 'Background image terpasang.', 'success');
                syncBackgroundImageControls();
            }, {
                crossOrigin: 'anonymous'
            });
        ...
        function uploadCanvasBackgroundImage(file) {
            ...
            const isGif = isGifSource(file.name || '', file.type || '');
            const maxSize = isGif ? 3 * 1024 * 1024 : 1024 * 1024;
            if (file.size > maxSize) {
                setBackgroundImageStatus(isGif ? 'Upload dibatalkan. Maksimal background GIF 3MB.' : 'Upload dibatalkan. Maksimal background 1MB.', 'error');
                setStatus(isGif ? 'Maksimal background GIF 3MB' : 'Maksimal background 1MB', 'error');
                return;
            }
            ...
            if (isGif) {
                setBackgroundImageStatus('Mengupload background GIF...', '');
                uploadSingleImage(file).then(asset => {
                    if (!asset?.src) {
                        throw new Error('URL background GIF tidak ditemukan.');
                    }
                    setCanvasBackgroundImage(asset.src, {
                        mime: asset.mime || asset.mimeType || file.type || '',
                        aaAnimatedSrc: asset.aaAnimatedSrc || asset.src,
                    });
                }).catch(error => {
                    const reader = new FileReader();
                    reader.onload = () => {
                        setCanvasBackgroundImage(String(reader.result || ''), {
                            mime: file.type || 'image/gif',
                        });
                        setBackgroundImageStatus('Background GIF terpasang. File akan disimpan saat save/publish.', 'success');
                    };
                    reader.onerror = () => {
                        setBackgroundImageStatus(error.message || 'Background GIF gagal diupload.', 'error');
                        setStatus(error.message || 'Background GIF gagal diupload.', 'error');
                    };
                    setBackgroundImageStatus('Upload GIF gagal, memakai mode aman lokal...', '');
        ...
        function removeCanvasBackgroundImage() {
            ...
            state.canvas.requestRenderAll();
            snapshot();
            setBackgroundImageStatus(state.editMode === 'photobooth' ? 'Background frame Photobooth dihapus.' : 'Background image dihapus.', 'success');
            syncBackgroundImageControls();
        }
	        ...
	        function isGalleryPhotoObject(object) {
		        ...
		        function getGalleryPhotoReplaceTarget() {
		            const active = typeof getInteractionApplyTarget === 'function'
		                ? getInteractionApplyTarget(isGalleryPhotoInteractionObject)
		                : state.canvas?.getActiveObject?.();
	            if (isGalleryPhotoObject(active)) return active;
	            const stored = state.__aaGalleryPhotoReplaceTarget || null;
	            if (stored && isGalleryPhotoObject(stored) && state.canvas?.getObjects?.().includes(stored)) {
	                state.canvas.setActiveObject(stored);
	                state.canvas.requestRenderAll?.();
	                return stored;
		            }
			        ...
			        function showMobileReplaceFeedback(message, tone = 'saving', title = 'Memproses') {
			            if (!window.matchMedia('(max-width: 767px)').matches) return;
			            showEditorToast(message, tone, title);
			        }

				        function openMobileMediaLibraryForPick(message) {
				            if (typeof openLeftDrawerPanel === 'function') {
				                openLeftDrawerPanel('upload');
				            }
			        ...
			        function beginGalleryPhotoReplaceMode() {
			            if (guardPremiumFeature()) return false;
			            const target = getGalleryPhotoReplaceTarget();
	            if (!target) {
	                setStatus('Pilih element Gallery Photo terlebih dahulu.', 'error');
	                return false;
	            }
	            if (target.locked === true) {
	                setStatus('Gallery Photo terkunci. Unlock dulu untuk mengganti foto.', 'error');
	                return false;
	            }
		            state.__aaGalleryPhotoReplaceTarget = target;
				            state.canvas.setActiveObject(target);
				            state.canvas.requestRenderAll();
				            state.mediaMode = 'gallery-photo-replace';
				            hidePendingEditorToast();
				            setStatus('Pilih foto pengganti dari perangkat atau Media Library.');
				            return true;
				        }

	        function replaceGalleryPhotoAsset(asset) {
	            if (!asset || !asset.src) return false;
	            const target = getGalleryPhotoReplaceTarget();
	            if (!target) {
	                setStatus('Pilih element Gallery Photo terlebih dahulu.', 'error');
	                state.mediaMode = 'insert';
	                state.__aaGalleryPhotoReplaceTarget = null;
	                return false;
	            }
			            setStatus('Mengganti foto gallery...', 'saving');
			            setMediaUploadState('Mengganti foto gallery...');
			            showMobileReplaceFeedback('Mengganti foto gallery...', 'saving', 'Memproses');
			            insertImage(asset.src, {
		                replaceTarget: target,
		                customType: 'gallery-photo',
	                isGalleryPhoto: true,
		                galleryZoom: target.galleryZoom !== false,
		                galleryImageSrc: target.galleryZoom !== false ? asset.src : '',
		                galleryImageName: asset.name || 'Gallery',
		                mime: asset.mime || asset.mimeType || '',
			                onReplaceComplete: () => {
			                    setStatus('Foto gallery diganti');
			                    setMediaUploadState('Foto gallery diganti');
			                    showMobileReplaceFeedback('Foto gallery diganti', 'success', 'Berhasil');
			                    window.setTimeout(() => setMediaUploadState(''), 900);
			                },
		            });
		            state.__aaGalleryPhotoReplaceTarget = null;
		            return true;
	        ...
	        function isMobileReplaceableImageObject(object) {
	            return Boolean(object && object.type === 'image' &&
	                object.customType !== 'background' &&
	                !isGalleryPhotoObject(object)
	            );
	        }

	        function getMobileImageReplaceTarget() {
	            const active = typeof getInteractionApplyTarget === 'function'
	                ? getInteractionApplyTarget(isMobileImageInteractionObject)
	                : state.canvas?.getActiveObject?.();
	            if (isMobileReplaceableImageObject(active)) return active;
	            const stored = state.__aaMobileImageReplaceTarget || null;
	            if (stored && isMobileReplaceableImageObject(stored) && state.canvas?.getObjects?.().includes(stored)) {
	                state.canvas.setActiveObject(stored);
	                state.canvas.requestRenderAll?.();
	                return stored;
	            }
		        ...
		        function beginMobileImageReplaceMode() {
	            if (guardPremiumFeature()) return false;
	            const target = getMobileImageReplaceTarget();
	            if (!target) {
	                setStatus('Pilih gambar terlebih dahulu.', 'error');
	                return false;
	            }
	            if (target.locked === true) {
		            ...
		            state.__aaMobileImageReplaceTarget = target;
				            state.canvas.setActiveObject(target);
				            state.canvas.requestRenderAll();
				            state.mediaMode = 'image-replace';
				            hidePendingEditorToast();
				            setStatus('Pilih gambar pengganti dari perangkat atau Media Library.');
				            return true;
				        }

		        function getMobileFramePhotoTarget() {
		            const active = typeof getInteractionApplyTarget === 'function'
		                ? getInteractionApplyTarget(isFramePhotoInteractionObject)
		                : state.canvas?.getActiveObject?.();
		            if (typeof isFramePhotoInteractionObject === 'function' && isFramePhotoInteractionObject(active)) return active;
		            const stored = state.__aaMobileInteractionDrawerTarget || state.__aaFramePlaceholderFillTarget || null;
		            if (
		                stored &&
		                typeof isFramePhotoInteractionObject === 'function' &&
		                isFramePhotoInteractionObject(stored) &&
		                state.canvas?.getObjects?.().includes(stored)
		            ) {
		                state.canvas.setActiveObject(stored);
		                state.canvas.requestRenderAll?.();
		                return stored;
		        ...
		        function beginMobileFramePhotoFillMode() {
		            const target = getMobileFramePhotoTarget();
		            if (!target) {
		                setStatus('Pilih Frame Photo terlebih dahulu.', 'error');
		                return false;
		            }
		            if (target.locked === true) {
		                setStatus('Frame terkunci. Unlock dulu untuk mengganti foto.', 'error');
		                return false;
		            }
		            if (typeof isFramePlaceholderObject === 'function' && isFramePlaceholderObject(target)) {
		                return beginFramePlaceholderFillMode();
		            }
		            if (guardPremiumFeature()) return false;
		            state.__aaMobileImageReplaceTarget = target;
		            state.__aaFramePhotoReplaceTarget = target;
		            state.canvas.setActiveObject(target);
		            state.canvas.requestRenderAll();
		            state.mediaMode = 'image-replace';
		            hidePendingEditorToast();
		            setStatus('Pilih foto dari perangkat atau Media Library.');
		            return true;
		        }

		        function applyMobileFramePhotoShape(shapeName) {
		            const target = getMobileFramePhotoTarget();
		            if (!target) {
		                setStatus('Pilih Frame Photo terlebih dahulu.', 'error');
		                return false;
		            ...
		            if (target.locked === true) {
		                setStatus('Frame terkunci. Unlock dulu untuk mengubah bentuk.', 'error');
		                return false;
		            }
		            if (typeof isFramePlaceholderObject === 'function' && isFramePlaceholderObject(target)) {
		                return updateFramePlaceholderShape(target, shapeName);
		            }
		            state.canvas.setActiveObject(target);
		            applyImageFrameShape(shapeName);
		            syncInteractionPopover?.(state.canvas.getActiveObject());
		            return true;
		        }

		        function replaceMobileImageAsset(asset) {
		            if (!asset || !asset.src) return false;
		            const target = getMobileImageReplaceTarget();
		            const isFramePhotoReplace = state.__aaFramePhotoReplaceTarget === target;
	            if (!target) {
	                setStatus('Pilih gambar terlebih dahulu.', 'error');
	                state.mediaMode = 'insert';
	                state.__aaMobileImageReplaceTarget = null;
	                state.__aaFramePhotoReplaceTarget = null;
	                return false;
	            }
			            setStatus(isFramePhotoReplace ? 'Mengganti foto frame...' : 'Mengganti gambar...', 'saving');
			            setMediaUploadState(isFramePhotoReplace ? 'Mengganti foto frame...' : 'Mengganti gambar...');
			            showMobileReplaceFeedback(isFramePhotoReplace ? 'Mengganti foto frame...' : 'Mengganti gambar...', 'saving', 'Memproses');
			            insertImage(asset.src, {
		                replaceTarget: target,
		                customType: target.customType || 'image',
		                mime: asset.mime || asset.mimeType || '',
		                aaOriginalImageSrc: asset.src,
		                aaOriginalImageName: asset.name || 'Gambar',
			                onReplaceComplete: () => {
			                    setStatus(isFramePhotoReplace ? 'Foto frame diganti' : 'Gambar diganti');
			                    setMediaUploadState(isFramePhotoReplace ? 'Foto frame diganti' : 'Gambar diganti');
			                    showMobileReplaceFeedback(isFramePhotoReplace ? 'Foto frame diganti' : 'Gambar diganti', 'success', 'Berhasil');
			                    window.setTimeout(() => setMediaUploadState(''), 900);
			                },
		            });
			            state.__aaMobileImageReplaceTarget = null;
		        ...
		        function fillFramePlaceholderAsset(asset) {
		            if (!asset || !asset.src) return false;
		            const target = getFramePlaceholderTarget();
		            if (!target) {
		                setStatus('Pilih frame kosong terlebih dahulu.', 'error');
		                state.mediaMode = 'insert';
		                state.__aaFramePlaceholderFillTarget = null;
		                return false;
		            }
		            setStatus('Mengisi foto ke frame...', 'saving');
		            setMediaUploadState('Mengisi foto ke frame...');
		            showMobileReplaceFeedback('Mengisi foto ke frame...', 'saving', 'Frame Photo');
		            insertImage(asset.src, {
		                replaceTarget: target,
		                mime: asset.mime || asset.mimeType || '',
		                aaOriginalImageSrc: asset.src,
		                ...
		                onReplaceComplete: () => {
		                    setStatus('Foto masuk ke frame');
		                    setMediaUploadState('Foto masuk ke frame');
		                    showMobileReplaceFeedback('Foto masuk ke frame', 'success', 'Berhasil');
		                    window.setTimeout(() => setMediaUploadState(''), 900);
		                },
		            });
		            state.__aaFramePlaceholderFillTarget = null;
            ...
            const allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(mime) ||
                /\.(jpe?g|png|webp|gif)$/i.test(name);
            if (!allowed) {
                throw new Error(`${file.name || 'Gambar'} harus JPG, PNG, WEBP, atau GIF.`);
            }
            ...
            if (asset && asset.src) {
                return normalizeGalleryItem({
                    ...
                    mime: asset.mime || asset.mimeType || file.type || '',
                    mimeType: asset.mimeType || asset.mime || file.type || '',
                    ...gifMetadataForSource(asset.src, asset.mime || asset.mimeType || file.type || ''),
        ...
        function setMagicLayerUploadPreview(file) {
        ...
        async function uploadImages(files) {
	            ...
	            let fileList = Array.from(files || []);
			            if (state.mediaMode === 'gallery-photo' || state.mediaMode === 'gallery-photo-replace' ||
			                state.mediaMode === 'image-replace' || state.mediaMode === 'frame-placeholder-fill') {
			                fileList = fileList.slice(0, 1);
			            }
	            if (!fileList.length) {
	                hidePendingEditorToast();
	                setMediaUploadState('');
	                setStatus('Pemilihan foto dibatalkan.');
	                return;
	            }
            ...
            if (oversizedFiles.length) {
                ...
                return;
		            ...
		            if (state.mediaMode === 'gallery-photo-replace') {
		                showMobileReplaceFeedback('Mengupload foto pengganti...', 'saving', 'Gallery Photo');
		            } else if (state.mediaMode === 'image-replace') {
		                showMobileReplaceFeedback(state.__aaFramePhotoReplaceTarget ? 'Mengupload foto frame...' : 'Mengupload gambar pengganti...', 'saving', state.__aaFramePhotoReplaceTarget ? 'Frame Photo' : 'Ganti Gambar');
		            } else if (state.mediaMode === 'frame-placeholder-fill') {
		                setStatus('Mengupload foto untuk frame...', 'saving');
		                showMobileReplaceFeedback('Mengupload foto frame...', 'saving', 'Frame Photo');
		            }
            ...
            await loadMedia({ force: true });
	            if (state.mediaMode === 'gallery-photo') {
		            ...
		            } else if (state.mediaMode === 'gallery-photo-replace') {
		                if (!canUsePremiumFeature()) {
		                    state.mediaMode = 'insert';
		                    state.__aaGalleryPhotoReplaceTarget = null;
		                } else {
		                    setStatus('Memproses foto gallery...', 'saving');
		                    setMediaUploadState('Memproses foto gallery...');
		                    replaceGalleryPhotoAsset(uploadedAssets[0]);
		                    state.mediaMode = 'insert';
		                }
			            } else if (state.mediaMode === 'image-replace') {
			                if (!canUsePremiumFeature()) {
		                    ...
		                    setStatus('Memproses gambar pengganti...', 'saving');
		                    setMediaUploadState('Memproses gambar pengganti...');
				                    replaceMobileImageAsset(uploadedAssets[0]);
				                    state.mediaMode = 'insert';
			                }
			            } else if (state.mediaMode === 'frame-placeholder-fill') {
			                setStatus('Memproses foto frame...', 'saving');
			                setMediaUploadState('Memproses foto frame...');
			                showMobileReplaceFeedback('Memproses foto frame...', 'saving', 'Frame Photo');
			                fillFramePlaceholderAsset(uploadedAssets[0]);
			                state.mediaMode = 'insert';
        ...
        async function loadMedia(options = {}) {
            ...
            const mediaThumbRowHeight = 4;
            const mediaThumbGap = 8;
            const mediaThumbMinHeight = 84;
            const mediaThumbMaxHeight = 184;

            function setMediaThumbnailHeight(card, targetHeight) {
                if (!card) return;
                const height = Math.max(mediaThumbMinHeight, Math.min(mediaThumbMaxHeight, Number(targetHeight) || 112));
                const span = Math.max(8, Math.min(18, Math.ceil((height + mediaThumbGap) / (mediaThumbRowHeight + mediaThumbGap))));
                card.style.setProperty('--aa-media-row-span', String(span));
            }

            function isTransparentMediaCandidate(item) {
                const mime = String(item?.mime || item?.mimeType || '').toLowerCase();
                const name = String(item?.name || item?.src || '').toLowerCase();
            ...
            function measureVisibleAlphaScale(image) {
                ...
                try {
                    ...
                    const visibleMaxRatio = Math.max(visibleWidthRatio, visibleHeightRatio);
                    const visibleAreaRatio = visibleWidthRatio * visibleHeightRatio;
                    if (visibleMaxRatio >= .82 || visibleAreaRatio >= .58) return 1;

                    return Math.min(2.35, Math.max(1, .82 / Math.max(.08, visibleMaxRatio)));
                } catch (error) {
                    return 1;
                }
            }

            function scheduleTransparentMediaPreview(card, image, item) {
                if (!isTransparentMediaCandidate(item) || card?.dataset.aaTransparentPreviewReady === '1') return;
                card.dataset.aaTransparentPreviewReady = '1';
                const run = () => {
                    const scale = measureVisibleAlphaScale(image);
                    if (scale > 1.04) {
                        card.style.setProperty('--aa-media-preview-scale', scale.toFixed(2));
                        card.classList.add('is-transparent-preview');
                    }
                };
            ...
            function syncMediaThumbnailPreview(card, image, item) {
                const naturalWidth = image?.naturalWidth || 1;
                const naturalHeight = image?.naturalHeight || 1;
                const cardWidth = Math.max(72, card?.getBoundingClientRect?.().width || image?.getBoundingClientRect?.().width || 96);
                const ratio = Math.max(.55, Math.min(1.8, naturalHeight / naturalWidth));
                setMediaThumbnailHeight(card, cardWidth * ratio);
                scheduleTransparentMediaPreview(card, image, item);
            ...
            media.forEach(item => {
                ...
                mediaImage.addEventListener('load', () => {
                    syncMediaThumbnailPreview(card, mediaImage, item);
                }, {
                    once: true
                });
                pickButton.appendChild(mediaImage);
                    ...
                    const active = state.canvas.getActiveObject();
	                    if (state.mediaMode === 'gallery-photo') {
		                    ...
		                    if (state.mediaMode === 'gallery-photo-replace') {
		                        ...
		                        setStatus('Memproses foto gallery...', 'saving');
		                        setMediaUploadState('Memproses foto gallery...');
		                        replaceGalleryPhotoAsset(item);
			                        state.mediaMode = 'insert';
			                        return;
			                    }
			                    if (state.mediaMode === 'image-replace') {
			                        if (!canUsePremiumFeature()) {
			                            openEditorAccessModal();
			                            state.mediaMode = 'insert';
			                            state.__aaMobileImageReplaceTarget = null;
				                        ...
				                        setStatus('Memproses gambar pengganti...', 'saving');
				                        setMediaUploadState('Memproses gambar pengganti...');
				                        replaceMobileImageAsset(item);
				                        state.mediaMode = 'insert';
				                        return;
				                    }
			                    if (state.mediaMode === 'frame-placeholder-fill') {
			                        setStatus('Memproses foto frame...', 'saving');
			                        setMediaUploadState('Memproses foto frame...');
			                        fillFramePlaceholderAsset(item);
			                        state.mediaMode = 'insert';
			                        return;
			                    }
                ...
                if (mediaImage.complete && mediaImage.naturalWidth) {
                    requestAnimationFrame(() => syncMediaThumbnailPreview(card, mediaImage, item));
                }
            });
            requestAnimationFrame(syncMediaMenuPlacement);
            syncMediaBulkBar(media);
        ...
        function duplicateActive() {
            ...
            if (typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(active)) {
                setStatus('Slot foto Photobooth tidak bisa diduplikasi agar jumlah slot tetap sesuai frame.', 'error');
                return false;
            }
        ...
        function deleteActive() {
            ...
            if (typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(active)) {
                setStatus('Slot foto Photobooth wajib ada dan tidak bisa dihapus.', 'error');
                state.canvas.discardActiveObject();
                state.canvas.requestRenderAll();
                syncInspector();
                return;
            ...
            if (active.type === 'activeSelection' && typeof active.getObjects === 'function') {
                const protectedObjects = active.getObjects().filter(object => typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(object));
                const objects = active.getObjects().filter(object => object && object !== state.cropBox && !(typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(object)));
                state.canvas.discardActiveObject();
                objects.forEach(object => {
                ...
                if (protectedObjects.length && !objects.length) {
                    setStatus('Slot foto Photobooth wajib ada dan tidak bisa dihapus.', 'error');
                }
            } else {
                state.canvas.remove(active);
        ...
        function setObjectLocked(object, locked) {
            if (!object) return;
            if (typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(object)) {
                setStatus('Slot foto Photobooth tetap aktif agar bisa digeser dan diresize.', 'error');
                return;
            }
            object.set({
        ...
        function aaIsLayerPanelObject(object) {
            if (!object) return false;
            if (object === state.cropBox) return false;
            if (object.__aaSkipObject === true) return false;
            if (object.excludeFromLayer === true || object.excludeFromLayering === true) return false;
            if (object.customType === 'background' || object.customType === 'guide' || object.customType === 'helper' ||
                object.customType === 'selection-helper' || object.customType === 'crop-helper') return false;
            if (object.name === 'Background Image' || object.name === 'background' || object.id === 'background') return false;
            return true;
        }

        function aaLayerObjectLabel(object) {
            if (!object) return 'Object';
            if (typeof aaIsPhotoboothPhotoSlot === 'function' && aaIsPhotoboothPhotoSlot(object)) {
                return `Slot Foto ${Number(object.aaPhotoboothSlotIndex || 1) || 1}`;
            }
            const explicit = String(object.label || object.name || object.placeholder || '').trim();
            if (explicit && explicit !== 'Background Image') return explicit.slice(0, 42);
            if (object.text) return String(object.text).replace(/\s+/g, ' ').trim().slice(0, 42) || 'Teks';
            if (object.customType === 'music-player') return 'Music Player';
            if (object.customType === 'countdown-timer') return 'Countdown';
            if (object.customType === 'youtube-video') return 'YouTube';
            if (object.customType === 'photo-gallery') return 'Gallery';
            if (object.customType === 'social-media') return 'Social Media';
            if (object.type === 'image') return 'Gambar';
            if (object.type === 'textbox' || object.type === 'i-text' || object.type === 'text') return 'Teks';
            if (object.type === 'rect') return 'Shape';
            if (object.type === 'circle') return 'Circle';
            if (object.type === 'group') return 'Group';
            return String(object.customType || object.type || 'Object').replace(/[-_]/g, ' ');
        }

        function aaLayerObjectIcon(object) {
            if (!object) return 'fa-shapes';
            if (typeof aaIsPhotoboothPhotoSlot === 'function' && aaIsPhotoboothPhotoSlot(object)) return 'fa-crop-simple';
            if (object.customType === 'music-player') return 'fa-music';
            if (object.customType === 'countdown-timer') return 'fa-clock';
            if (object.customType === 'youtube-video') return 'fa-play';
            if (object.customType === 'photo-gallery') return 'fa-images';
            if (object.customType === 'social-media') return 'fa-share-nodes';
            if (object.type === 'image') return 'fa-image';
            if (object.type === 'textbox' || object.type === 'i-text' || object.type === 'text') return 'fa-font';
            if (object.type === 'group') return 'fa-layer-group';
            return 'fa-shapes';
        }

        function aaGetLayerPanelObjects() {
            if (!state.canvas || typeof state.canvas.getObjects !== 'function') return [];
            return state.canvas.getObjects().filter(aaIsLayerPanelObject);
        }

        function aaScheduleLayerPanelRender() {
            if (state.__aaLayerPanelRenderFrame) return;
            state.__aaLayerPanelRenderFrame = requestAnimationFrame(function() {
                state.__aaLayerPanelRenderFrame = null;
                aaRenderLayerPanel();
            });
        }

        function aaRenderLayerPanel() {
            if (!els.aaLayerList) return;
            const objects = aaGetLayerPanelObjects();
            const active = state.canvas?.getActiveObject?.();
            if (els.aaLayerCount) {
                els.aaLayerCount.textContent = String(objects.length);
            }
            if (!objects.length) {
                els.aaLayerList.innerHTML = '<div class="aa-layer-empty">Belum ada object di halaman ini.</div>';
                return;
            }

            els.aaLayerList.innerHTML = objects.slice().reverse().map(function(object) {
                const index = objects.indexOf(object);
                const realIndex = state.canvas.getObjects().indexOf(object);
                const selected = object === active || (active?.type === 'activeSelection' && active.getObjects?.().includes(object));
                const hidden = object.visible === false;
                const locked = object.locked === true;
                const protectedPhotobooth = typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(object);
                const protectedDisabled = protectedPhotobooth ? ' disabled aria-disabled="true"' : '';
                return `
                    <div class="aa-layer-row ${selected ? 'is-active' : ''} ${hidden ? 'is-hidden-object' : ''}" role="listitem" data-aa-layer-index="${realIndex}">
                        <button class="aa-layer-main" type="button" data-aa-layer-select="${realIndex}" title="Pilih object">
                            <span class="aa-layer-icon"><i class="fa ${aaLayerObjectIcon(object)}" aria-hidden="true"></i></span>
                            <span class="aa-layer-meta">
                                <strong>${escapeHtml(aaLayerObjectLabel(object))}</strong>
                                <small>${hidden ? 'Hidden' : `Layer ${index + 1}`}</small>
                            </span>
                        </button>
                        <span class="aa-layer-actions">
                            <button type="button" data-aa-layer-panel-action="toggle-visible" data-aa-layer-index="${realIndex}" title="${protectedPhotobooth ? 'Slot foto wajib tampil' : (hidden ? 'Tampilkan' : 'Sembunyikan')}"${protectedDisabled}><i class="fa ${hidden ? 'fa-eye-slash' : 'fa-eye'}" aria-hidden="true"></i></button>
                            <button type="button" data-aa-layer-panel-action="toggle-lock" data-aa-layer-index="${realIndex}" title="${protectedPhotobooth ? 'Slot foto tetap bisa diedit' : (locked ? 'Unlock' : 'Lock')}"${protectedDisabled}><i class="fa ${locked ? 'fa-lock' : 'fa-lock-open'}" aria-hidden="true"></i></button>
                            <button type="button" data-aa-layer-panel-action="bring-forward" data-aa-layer-index="${realIndex}" title="Naik"><i class="fa fa-arrow-up" aria-hidden="true"></i></button>
                            <button type="button" data-aa-layer-panel-action="send-backward" data-aa-layer-index="${realIndex}" title="Turun"><i class="fa fa-arrow-down" aria-hidden="true"></i></button>
                        </span>
                    </div>
                `;
            }).join('');
        ...
        function aaGetLayerPanelObjectByIndex(index) {
            const realIndex = Number(index);
            if (!state.canvas || !Number.isInteger(realIndex)) return null;
            return state.canvas.getObjects()[realIndex] || null;
        }

        function aaHandleLayerPanelClick(event) {
            const selectButton = event.target.closest('[data-aa-layer-select]');
            const actionButton = event.target.closest('[data-aa-layer-panel-action]');
            const button = actionButton || selectButton;
            if (!button) return;
            event.preventDefault();
            event.stopPropagation();

            const object = aaGetLayerPanelObjectByIndex(button.dataset.aaLayerIndex || button.dataset.aaLayerSelect);
            if (!object || !state.canvas) return;

            if (actionButton) {
                const action = actionButton.dataset.aaLayerPanelAction || '';
                const protectedPhotobooth = typeof aaIsProtectedPhotoboothObject === 'function' && aaIsProtectedPhotoboothObject(object);
                if (protectedPhotobooth && (action === 'toggle-visible' || action === 'toggle-lock')) {
                    setStatus('Slot foto Photobooth wajib aktif dan tidak bisa disembunyikan.', 'error');
                    return;
                }
                ...
                if (action === 'toggle-lock') {
                    setObjectLocked(object, object.locked !== true);
                    state.canvas.setActiveObject(object);
                    state.canvas.requestRenderAll();
                    storeCurrentPage();
                    snapshot();
                ...
                state.canvas.setActiveObject(object);
                state.canvas.requestRenderAll();
                if (typeof window.aaSmartLayerAction === 'function' &&
                    (action === 'bring-forward' || action === 'send-backward')) {
                    window.aaSmartLayerAction(action);
                } else if (action === 'bring-forward') {
                    state.canvas.bringForward(object);
                    storeCurrentPage();
                    snapshot();
                    syncInspector();
                } else if (action === 'send-backward') {
                    state.canvas.sendBackwards(object);
                    storeCurrentPage();
                    snapshot();
                    syncInspector();
                }
            ...
            if (object.visible === false) {
                setStatus('Object sedang hidden. Klik ikon mata untuk menampilkan.', 'error');
                return;
            }
            state.canvas.setActiveObject(object);
            object.setCoords?.();
        ...
        function shouldCenterGroupAnimationOrigin(animationName, object) {
        ...
        function previewObjectAnimation(animationName, object) {
            ...
            const restoreOriginal = getAnimationSnapshot(object);
            if (shouldCenterGroupAnimationOrigin(animationName, object)) {
            ...
            const finish = () => {
                restoreAnimationSnapshot(object, restoreOriginal);
        ...
        function updateActiveTextAnimationTiming(commit = false, sourceInput = null) {
            if (aaIsAnimationDisabledForCurrentMode()) return;
        ...
        function showTemplateCategoryView() {
            ...
            if (els.aaTemplateListView) {
                ...
                els.aaTemplateListView.dataset.aaSelectedTemplateCategory = 'all';
            }
            if (els.aaTemplateSearchInput) {
                els.aaTemplateSearchInput.value = '';
            }
            if (els.aaTemplateCurrentCategoryTitle) {
                els.aaTemplateCurrentCategoryTitle.textContent = 'Semua Template';
            }
            syncTemplateCategoryChips('all');
            closeTemplateDetailPopover();
            filterTemplateDrawer();
            window.requestAnimationFrame(updateTemplateChipScrollState);
        }
        ...
        function showTemplateListView(category, label) {
            ...
            if (els.aaTemplateListView) {
                els.aaTemplateListView.hidden = false;
                els.aaTemplateListView.dataset.aaSelectedTemplateCategory = category || 'all';
            }
            if (els.aaTemplateCurrentCategoryTitle) {
                els.aaTemplateCurrentCategoryTitle.textContent = category === 'all' ? 'Semua Template' : (label || 'Template');
            }
            if (els.aaTemplateSearchInput) {
                els.aaTemplateSearchInput.value = '';
            }
            syncTemplateCategoryChips(category || 'all');
            closeTemplateDetailPopover();
            filterTemplateDrawer();
            window.requestAnimationFrame(updateTemplateChipScrollState);
        }

        function syncTemplateCategoryChips(activeCategory = 'all') {
            els.aaTemplateCategoryChips?.querySelectorAll('[data-aa-template-category]').forEach(button => {
                button.classList.toggle('is-active', String(button.dataset.aaTemplateCategory || 'all') === String(activeCategory || 'all'));
            });
            if (els.aaTemplateBackBtn) {
                els.aaTemplateBackBtn.hidden = String(activeCategory || 'all') === 'all';
            }
            updateTemplateChipScrollState();
        }

        function updateTemplateChipScrollState() {
            const scroller = els.aaTemplateCategoryChips;
            if (!scroller) return;
            const wrap = scroller.closest('[data-aa-template-chip-wrap]');
            if (!wrap) return;
            const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
            const canLeft = scroller.scrollLeft > 2;
            const canRight = scroller.scrollLeft < maxScroll - 2;
            wrap.querySelectorAll('[data-aa-template-chip-scroll]').forEach(button => {
                const direction = Number(button.dataset.aaTemplateChipScroll || 0);
                button.disabled = direction < 0 ? !canLeft : !canRight;
            });
        }

        function scrollTemplateCategoryChips(direction = 1) {
            const scroller = els.aaTemplateCategoryChips;
            if (!scroller) return;
            const amount = Math.max(96, Math.floor(scroller.clientWidth * .62));
            scroller.scrollBy({
                left: direction * amount,
                behavior: 'smooth'
            });
            window.setTimeout(updateTemplateChipScrollState, 180);
        }
        ...
        function filterTemplateDrawer() {
            ...
            const activeCategory = els.aaTemplateListView?.dataset?.aaSelectedTemplateCategory || 'all';
            let visibleCount = 0;
            closeTemplateDetailPopover();
            ...
            cards.forEach(card => {
                ...
                const matchesCategory = activeCategory === 'all' || card.dataset.aaTemplateCategory === activeCategory;
        ...
        function closeTemplateDetailPopover() {
            if (!els.aaTemplateDetailPopover) return;
            els.aaTemplateDetailPopover.hidden = true;
            els.aaTemplateDetailPopover.innerHTML = '';
        }

        function openTemplateDetailPopover(card, trigger) {
            if (!card || !trigger || !els.aaTemplateDetailPopover) return;
            const title = card.dataset.aaTemplateTitle || 'Template';
            const category = card.dataset.aaTemplateCategoryName || 'Lainnya';
            const isPremium = card.dataset.aaTemplatePremium === '1';
            els.aaTemplateDetailPopover.innerHTML = `
                <strong>${escapeHtml(title)}</strong>
                <span>${escapeHtml(category)}</span>
                <em>${isPremium ? 'Premium' : 'Free'}</em>
                <button type="button" data-aa-template-detail-preview="${escapeHtml(card.dataset.aaTemplatePreview || '')}">Preview</button>
            `;
            const wrap = els.aaTemplateListView?.getBoundingClientRect();
            const rect = trigger.getBoundingClientRect();
            if (wrap) {
                els.aaTemplateDetailPopover.style.left = `${Math.max(8, Math.min(rect.left - wrap.left - 120, wrap.width - 188))}px`;
                els.aaTemplateDetailPopover.style.top = `${Math.max(8, rect.bottom - wrap.top + 8)}px`;
            }
            els.aaTemplateDetailPopover.hidden = false;
            els.aaTemplateDetailPopover.__aaTemplateCard = card;
        }
        ...
        async function ensureEditorAssetLibrary() {
            ...
            state.editorAssetMeta = data.meta || null;
        ...
        function editorAssetCategoryIcon(label = '', slug = '') {
            ...
            if (text.includes('bank') || text.includes('dompet') || text.includes('payment') || text.includes('mandiri') ||
                text.includes('bca') || text.includes('seabank')) return 'fa-wallet';
            if (text.includes('abstract')) return 'fa-draw-polygon';
            if (text.includes('frame') || text.includes('border')) return 'fa-border-all';
            if (text.includes('divider')) return 'fa-grip-lines';
            if (text.includes('photo')) return 'fa-image';
            if (text.includes('pattern')) return 'fa-table-cells-large';
            if (text.includes('ribbon') || text.includes('badge')) return 'fa-certificate';
            if (text.includes('wedding floral') || text.includes('floral') || text.includes('flower')) return 'fa-seedling';
            if (text.includes('lamaran') || text.includes('wedding') || text.includes('nikah') || text.includes('pernikahan'))
                return 'fa-heart';
            if (text.includes('ultah') || text.includes('birthday') || text.includes('ulang')) return 'fa-cake-candles';
            if (text.includes('bubker') || text.includes('ramadan') || text.includes('eid') || text.includes('halal') ||
                text.includes('islam')) return 'fa-mosque';
            if (text.includes('aqiqah')) return 'fa-baby-carriage';
            if (text.includes('baby')) return 'fa-baby';
            ...
            if (text.includes('syukuran')) return 'fa-hands-praying';
            if (text.includes('wisuda')) return 'fa-graduation-cap';
            if (text.includes('batik') || text.includes('nusantara')) return 'fa-fan';
            if (text.includes('luxury') || text.includes('gold')) return 'fa-gem';
            if (text.includes('minimal')) return 'fa-wand-magic-sparkles';
            if (text.includes('shape')) return 'fa-shapes';
            return 'fa-shapes';
        }
        ...
        function showEditorAssetCategoryView() {
            ...
            if (els.aaEditorAssetGlobalSearchInput) els.aaEditorAssetGlobalSearchInput.value = '';
            if (els.aaEditorAssetMoreBtn) els.aaEditorAssetMoreBtn.hidden = true;
            renderEditorAssetLandingSections();
            window.requestAnimationFrame(updateEditorAssetQuickScrollState);
        }
        ...
        function showEditorAssetListView(category, label) {
            ...
            renderEditorAssetTypeChips();
            renderEditorAssets();
        }

        function showEditorAssetGlobalResults(query, label = '') {
            const cleanQuery = String(query || '').trim();
            if (!cleanQuery) {
                showEditorAssetCategoryView();
                return;
            }
            state.editorAssetCategory = 'all';
            state.editorAssetQuery = cleanQuery;
            state.editorAssetVisible = 40;
            if (els.aaEditorAssetSearchInput) els.aaEditorAssetSearchInput.value = cleanQuery;
            if (els.aaEditorAssetGlobalSearchInput) els.aaEditorAssetGlobalSearchInput.value = cleanQuery;
            if (els.aaEditorAssetCategoryView) els.aaEditorAssetCategoryView.hidden = true;
            if (els.aaEditorAssetListView) els.aaEditorAssetListView.hidden = false;
            if (els.aaEditorAssetCurrentCategoryTitle) {
                els.aaEditorAssetCurrentCategoryTitle.textContent = label || `Hasil: ${cleanQuery}`;
            }
            renderEditorAssetTypeChips();
            renderEditorAssets();
        }

        function openEditorAssetTypeSection(type, label) {
            state.editorAssetType = type || 'all';
            state.editorAssetCategory = 'all';
            state.editorAssetQuery = '';
            state.editorAssetVisible = 40;
            if (els.aaEditorAssetSearchInput) els.aaEditorAssetSearchInput.value = '';
        ...
        const AA_PHOTO_FRAME_CATEGORY = '__photo_frame';
        const AA_PHOTO_FRAME_PRESETS = [
            { shape: 'rounded', name: 'Rounded' },
            { shape: 'circle', name: 'Circle' },
            { shape: 'arch', name: 'Arch' },
            { shape: 'heart', name: 'Love' },
            { shape: 'diamond', name: 'Diamond' },
            { shape: 'blob', name: 'Blob' },
            { shape: 'ticket', name: 'Ticket' },
            { shape: 'oval', name: 'Oval' },
            { shape: 'shield', name: 'Shield' },
            { shape: 'hexagon', name: 'Hexagon' },
            { shape: 'petal', name: 'Petal' },
            { shape: 'wave', name: 'Wave' },
            { shape: 'tag', name: 'Tag' },
            { shape: 'bookmark', name: 'Bookmark' },
            { shape: 'scallop', name: 'Scallop' },
        ];

        function renderPhotoFramePresetButton(preset, mode = 'preview') {
            const button = document.createElement('button');
            button.type = 'button';
            button.className = mode === 'grid' ? 'aa-editor-asset-pick aa-photo-frame-preset-pick' : 'aa-editor-asset-preview-card aa-photo-frame-preset-card';
            button.dataset.aaFramePreset = preset.shape;
            button.title = preset.name;
            button.innerHTML = `
                <span class="aa-photo-frame-preset-preview is-${escapeHtml(preset.shape)}" aria-hidden="true"></span>
                <span class="aa-photo-frame-preset-name">${escapeHtml(preset.name)}</span>
            `;
            return button;
        }

        function insertPhotoFramePreset(shapeName) {
            const shape = String(shapeName || 'rounded');
            if (!AA_PHOTO_FRAME_PRESETS.some(item => item.shape === shape)) return;
            addImageFramePlaceholder(shape);
        }

        function renderEditorAssetLandingSections() {
            if (!els.aaEditorAssetLandingSections) return;
            els.aaEditorAssetLandingSections.hidden = false;
            const assetCreatedTime = asset => {
                const time = Date.parse(asset?.createdAt || '');
                return Number.isFinite(time) ? time : 0;
            };
            const sortByLatestAsset = (a, b) => {
                const createdDiff = assetCreatedTime(b) - assetCreatedTime(a);
                if (createdDiff !== 0) return createdDiff;
                return Number(b?.id || 0) - Number(a?.id || 0);
            };
            const sortByPopularAsset = (a, b) => {
                const usageDiff = Number(b?.usageCount || 0) - Number(a?.usageCount || 0);
                if (usageDiff !== 0) return usageDiff;
                return sortByLatestAsset(a, b);
            };
            const groups = [{
                    key: 'popular',
                    label: 'Populer',
                    filter: asset => true,
                    sort: sortByPopularAsset
                },
                {
                    key: 'ornament',
                    label: 'Graphics & Ornament',
                    filter: asset => asset.type === 'ornament',
                    sort: sortByLatestAsset
                },
                {
                    key: 'shape',
                    label: 'Shapes',
                ...
                },
                {
                    ...
                    filter: asset => asset.type === 'background' || asset.type === 'pattern',
                    sort: sortByLatestAsset
                }
            ];
            els.aaEditorAssetLandingSections.innerHTML = '';
            let renderedCount = 0;
            const frameSection = document.createElement('section');
            frameSection.className = 'aa-editor-asset-preview-section aa-photo-frame-section';
            const frameHead = document.createElement('div');
            frameHead.className = 'aa-editor-asset-preview-head';
            frameHead.innerHTML = '<strong>Photo Frame</strong>';
            const frameSeeAll = document.createElement('button');
            frameSeeAll.type = 'button';
            frameSeeAll.textContent = 'See all';
            frameSeeAll.dataset.aaEditorAssetCategory = AA_PHOTO_FRAME_CATEGORY;
            frameSeeAll.dataset.aaEditorAssetCategoryLabel = 'Photo Frame';
            frameHead.appendChild(frameSeeAll);
            const frameRow = document.createElement('div');
            frameRow.className = 'aa-editor-asset-preview-row';
            AA_PHOTO_FRAME_PRESETS.forEach(preset => {
                frameRow.appendChild(renderPhotoFramePresetButton(preset));
            });
            frameSection.appendChild(frameHead);
            frameSection.appendChild(frameRow);
            els.aaEditorAssetLandingSections.appendChild(frameSection);
            renderedCount += 1;
            groups.forEach(group => {
                const assets = state.editorAssets.filter(group.filter).sort(group.sort).slice(0, 10);
                if (!assets.length) return;
                renderedCount += 1;
                const section = document.createElement('section');
                section.className = 'aa-editor-asset-preview-section';
                const head = document.createElement('div');
                head.className = 'aa-editor-asset-preview-head';
                const seeAllType = group.key === 'popular' ? 'all' : group.key === 'background' ? 'background' :
                    group.key;
                head.innerHTML = `<strong>${escapeHtml(group.label)}</strong>`;
                const seeAll = document.createElement('button');
                seeAll.type = 'button';
                seeAll.textContent = 'See all';
                seeAll.dataset.aaEditorAssetOpenType = seeAllType;
                seeAll.dataset.aaEditorAssetOpenLabel = group.label;
                head.appendChild(seeAll);
                const row = document.createElement('div');
                row.className = 'aa-editor-asset-preview-row';
                assets.forEach(asset => {
                    const button = document.createElement('button');
                    button.type = 'button';
                    button.className = 'aa-editor-asset-preview-card';
                    button.dataset.aaEditorAssetId = asset.id;
                    button.title = asset.name || asset.id;
                    button.draggable = true;
                    button.innerHTML = `
                        <img src="${asset.thumbnail || asset.src}" loading="lazy" alt="">
                        ${asset.isPremium ? `<span class="aa-editor-asset-badge is-premium ${config.canUsePremiumFeatures ? 'is-unlocked' : 'is-locked'}" title="${config.canUsePremiumFeatures ? 'Premium aktif' : 'Fitur Premium'}"><i class="fa fa-crown" aria-hidden="true"></i></span>` : ''}
                    `;
                    button.addEventListener('dragstart', event => {
                        event.dataTransfer.effectAllowed = 'copy';
                        event.dataTransfer.setData('application/x-aa-editor-asset', String(asset.id || ''));
                    });
                    row.appendChild(button);
                });
                section.appendChild(head);
                section.appendChild(row);
                els.aaEditorAssetLandingSections.appendChild(section);
            });
            els.aaEditorAssetLandingSections.hidden = renderedCount === 0;
        }

        function renderEditorAssetTypeChips() {
            if (!els.aaEditorAssetTypeChips) return;
            els.aaEditorAssetTypeChips.querySelectorAll('[data-aa-editor-asset-type]').forEach(button => {
                button.classList.toggle('is-active', button.dataset.aaEditorAssetType === state.editorAssetType);
            });
            updateEditorAssetTypeScrollState();
        }

        function updateEditorAssetTypeScrollState() {
            const scroller = els.aaEditorAssetTypeChips;
            if (!scroller) return;
            const wrap = scroller.closest('[data-aa-editor-asset-type-wrap]');
            if (!wrap) return;
            const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
            const canLeft = scroller.scrollLeft > 2;
            const canRight = scroller.scrollLeft < maxScroll - 2;
            ...
            wrap.querySelectorAll('[data-aa-editor-asset-type-scroll]').forEach(button => {
                const direction = Number(button.dataset.aaEditorAssetTypeScroll || 0);
                button.disabled = direction < 0 ? !canLeft : !canRight;
            });
        }

        function scrollEditorAssetTypeChips(direction = 1) {
            const scroller = els.aaEditorAssetTypeChips;
            if (!scroller) return;
            const amount = Math.max(120, Math.floor(scroller.clientWidth * .72));
            scroller.scrollBy({
                left: direction * amount,
                behavior: 'smooth'
            });
            window.setTimeout(updateEditorAssetTypeScrollState, 180);
        }

        function updateEditorAssetQuickScrollState() {
            const scroller = els.aaEditorAssetQuickChips;
            if (!scroller) return;
            const wrap = scroller.closest('[data-aa-editor-asset-quick-wrap]');
            if (!wrap) return;
            const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
            const canLeft = scroller.scrollLeft > 2;
            const canRight = scroller.scrollLeft < maxScroll - 2;
            wrap.querySelectorAll('[data-aa-editor-asset-quick-scroll]').forEach(button => {
                const direction = Number(button.dataset.aaEditorAssetQuickScroll || 0);
                button.disabled = direction < 0 ? !canLeft : !canRight;
            });
        }

        function scrollEditorAssetQuickChips(direction = 1) {
            const scroller = els.aaEditorAssetQuickChips;
            if (!scroller) return;
            const amount = Math.max(96, Math.floor(scroller.clientWidth * .62));
            scroller.scrollBy({
            ...
            window.setTimeout(updateEditorAssetQuickScrollState, 180);
        }

        function renderEditorAssetCategoryCards() {
            if (!els.aaEditorAssetCategoryGrid) return;
            const categories = [{
                slug: AA_PHOTO_FRAME_CATEGORY,
                name: 'Photo Frame',
                count: AA_PHOTO_FRAME_PRESETS.length,
            }, ...getEditorAssetDrawerCategories()];
            els.aaEditorAssetCategoryGrid.innerHTML = '';
            ...
            categories.forEach((category, index) => {
                ...
                const button = document.createElement('button');
                button.type = 'button';
                button.className = 'aa-template-category-card aa-editor-asset-category-card';
        ...
        function filteredEditorAssets() {
            if (state.editorAssetCategory === AA_PHOTO_FRAME_CATEGORY) {
                const query = state.editorAssetQuery.trim().toLowerCase();
                return AA_PHOTO_FRAME_PRESETS.filter(preset => {
                    if (!query) return true;
                    return `${preset.shape} ${preset.name} photo frame`.toLowerCase().includes(query);
                });
            }
            const query = state.editorAssetQuery.trim().toLowerCase();
            return state.editorAssets.filter(asset => {
                if (state.editorAssetType === 'premium') {
                    if (asset.isPremium !== true) return false;
                } else if (state.editorAssetType !== 'all' && asset.type !== state.editorAssetType) {
                    return false;
                }
        ...
        function renderEditorAssets() {
            ...
            const typeFilterWrap = els.aaEditorAssetTypeChips?.closest('[data-aa-editor-asset-type-wrap]');
            if (typeFilterWrap) {
                typeFilterWrap.hidden = state.editorAssetCategory === AA_PHOTO_FRAME_CATEGORY;
            }
            els.aaEditorAssetGrid.classList.toggle('is-photo-frame-grid', state.editorAssetCategory === AA_PHOTO_FRAME_CATEGORY);
            const assets = filteredEditorAssets();
            const visible = assets.slice(0, state.editorAssetVisible);
            els.aaEditorAssetGrid.innerHTML = '';
            if (!visible.length) {
                els.aaEditorAssetGrid.innerHTML = `<div class="aa-gallery-empty">${state.editorAssetCategory === AA_PHOTO_FRAME_CATEGORY ? 'Frame photo tidak ditemukan' : 'Ornament terbaik lainnya sedang disiapkan'}</div>`;
            } else if (state.editorAssetCategory === AA_PHOTO_FRAME_CATEGORY) {
                visible.forEach(preset => {
                    const card = document.createElement('div');
                    card.className = 'aa-editor-asset-card aa-photo-frame-preset-wrap';
                    card.appendChild(renderPhotoFramePresetButton(preset, 'grid'));
                ...
                visible.forEach(asset => {
                    ...
                    if (String(state.editorAssetMenuId || '') === String(asset.id)) {
                    ...
                    pickButton.draggable = true;
                    pickButton.innerHTML = `<img src="${asset.thumbnail || asset.src}" loading="lazy" alt="">`;
                    pickButton.addEventListener('dragstart', event => {
                        event.dataTransfer.effectAllowed = 'copy';
                    ...
                    card.appendChild(pickButton);
		                    ...
		                    if (asset.isPremium) {
		                        ...
		                        badge.className = `aa-editor-asset-badge is-premium ${config.canUsePremiumFeatures ? 'is-unlocked' : 'is-locked'}`;
		                        badge.title = config.canUsePremiumFeatures ? 'Premium aktif' : 'Fitur Premium';
		                        badge.innerHTML = '<i class="fa fa-crown" aria-hidden="true"></i>';
		                        card.appendChild(badge);
                    } else if (asset.packName) {
                        const badge = document.createElement('span');
                        badge.className = 'aa-editor-asset-badge';
                        badge.textContent = asset.packName;
                        card.appendChild(badge);
                    }

                    if (config.canManageEditorAssets) {
                        const menuButton = document.createElement('button');
                        menuButton.type = 'button';
                        menuButton.className = 'aa-editor-asset-menu-toggle';
                        menuButton.dataset.aaEditorAssetMenuId = asset.id;
                        menuButton.title = 'Pengaturan asset';
                        menuButton.setAttribute('aria-label', `Pengaturan ${asset.name || 'asset'}`);
                        menuButton.innerHTML = '<i class="fa fa-ellipsis" aria-hidden="true"></i>';
                        ...
                        if (String(state.editorAssetMenuId || '') === String(asset.id)) {
                            const typeOptions = [{
                                ...
                                const selected = type.value === asset.type ? ' selected' : '';
                                return `<option value="${type.value}"${selected}>${type.label}</option>`;
                            }).join('');
                            const categoryOptions = state.editorAssetCategories.map(category => {
                                const slug = String(category.slug || category.name || '').trim();
                                if (!slug) return '';
                                const label = editorAssetDisplayCategoryName(category.name || slug, slug);
                                const selected = slug === asset.category ? ' selected' : '';
                                return `<option value="${escapeHtml(slug)}"${selected}>${escapeHtml(label)}</option>`;
                            }).filter(Boolean).join('');
                            const panel = document.createElement('div');
                            panel.className = 'aa-editor-asset-menu-panel';
                            panel.dataset.aaEditorAssetMenuPanel = asset.id;
                            panel.innerHTML = `
                                <label>
                                    <span>Tipe</span>
                                    <select data-aa-editor-asset-type-select="${escapeHtml(asset.id)}">
                                        ${typeOptions}
                                    </select>
                                </label>
                                <label>
                                    <span>Kategori</span>
                                    <select data-aa-editor-asset-category-select="${escapeHtml(asset.id)}">
                                        ${categoryOptions}
                                    </select>
                                </label>
                                <label class="aa-editor-asset-menu-check">
                                    <input type="checkbox" data-aa-editor-asset-premium-select="${escapeHtml(asset.id)}" ${asset.isPremium ? 'checked' : ''}>
                                    <span><i class="fa fa-crown" aria-hidden="true"></i> Premium</span>
                                </label>
                                <div class="aa-editor-asset-menu-actions">
                                    <button type="button" data-aa-editor-asset-menu-close>Cancel</button>
                                    <button type="button" data-aa-editor-asset-category-save="${escapeHtml(asset.id)}">Simpan</button>
                                </div>
                            `;
        ...
        async function loadAndRenderEditorAssets() {
            ...
            renderEditorAssetLandingSections();
            updateEditorAssetQuickScrollState();
            ...
            if (!config.canManageEditorAssets || !config.editorAssetDeleteUrl || !id) return;
            const asset = state.editorAssets.find(item => String(item.id) === String(id));
            if (!asset) {
                setStatus('Asset tidak ditemukan.', 'error');
                return;
            }
            if (!await aaConfirm(`Hapus asset "${asset.name || 'ini'}" dari library?`, {
                    title: 'Hapus Asset',
                    okText: 'Hapus',
                    cancelText: 'Batal',
                    danger: true
            ...
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok || data.success === false) {
                    throw new Error(data.message || 'Asset gagal dihapus.');
                }
                ...
                setStatus(data.message || 'Asset berhasil dihapus.');
            } catch (error) {
                setStatus(error.message || 'Asset gagal dihapus.', 'error');
                renderEditorAssets();
            }
        }

	        async function updateEditorAssetCategory(id, settings = {}) {
            const category = settings.category || '';
            const type = settings.type || 'ornament';
            const isPremium = settings.isPremium === true;
            if (!config.canManageEditorAssets || !config.editorAssetCategoryUrl || !id || !category) return;
            const asset = state.editorAssets.find(item => String(item.id) === String(id));
            if (!asset) {
                setStatus('Asset tidak ditemukan.', 'error');
                return;
            ...
            setStatus('Memperbarui pengaturan asset...', 'saving');
            const form = new FormData();
            form.append('category', category);
            form.append('type', type);
            form.append('is_premium', isPremium ? '1' : '0');
            if (config.editorAssetUploadToken) {
                form.append('admin_upload_token', config.editorAssetUploadToken);
            }

            try {
                const response = await fetch(`${config.editorAssetCategoryUrl}/${encodeURIComponent(id)}`, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
                ...
                if (!response.ok || data.success === false || !data.item) {
                    throw new Error(data.message || 'Pengaturan asset gagal diperbarui.');
                }

                state.editorAssets = state.editorAssets.map(item => String(item.id) === String(id) ? data.item : item);
                state.editorAssetMenuId = null;
                renderEditorAssetCategoryCards();
                renderEditorAssetLandingSections();
                if (state.editorAssetCategory) {
                    renderEditorAssets();
                ...
                setStatus(data.message || 'Pengaturan asset diperbarui.');
            } catch (error) {
                setStatus(error.message || 'Pengaturan asset gagal diperbarui.', 'error');
                renderEditorAssets();
            }
        }
	        ...
	        async function uploadSingleEditorAsset(file, type, category, isPremium = false) {
            const mime = String(file?.type || '').toLowerCase();
            const name = String(file?.name || '').toLowerCase();
            const allowed = ['image/svg+xml', 'image/png', 'image/jpeg', 'image/jpg', 'image/webp', 'image/gif'].includes(mime) ||
                /\.(svg|png|jpe?g|webp|gif)$/i.test(name);
            if (!allowed) {
                throw new Error(`${file?.name || 'Asset'} harus SVG, PNG, JPG, WEBP, atau GIF.`);
            }
            if (file?.size > 4 * 1024 * 1024) {
                throw new Error(`${file.name || 'Asset'} lebih besar dari batas 4MB.`);
            }
            const form = new FormData();
            form.append('type', type);
            form.append('category', category);
        ...
        async function uploadEditorAsset(event) {
            event?.preventDefault();
            if (!config.canManageEditorAssets || !config.editorAssetUploadUrl || !els.aaEditorAssetUploadFile) return;

            const category = els.aaEditorAssetUploadCategory?.value || '';
            const type = els.aaEditorAssetUploadType?.value || 'ornament';
            const isPremium = els.aaEditorAssetUploadPremium?.checked === true;
            ...
            try {
                for (const [index, file] of files.entries()) {
                    ...
                    try {
                        const item = await uploadSingleEditorAsset(file, type, category, isPremium);
                        if (item) {
        ...
        function placeEditorAssetObject(asset, object, options = {}) {
            ...
            let targetWidth = Math.min(420, canvasWidth * .56);
            let targetHeight = Math.min(420, canvasHeight * .28);
            ...
            state.canvas.requestRenderAll();
            snapshot();
            trackEditorAssetUsage(asset);
            setStatus('Asset ditambahkan');
        }

        function trackEditorAssetUsage(asset) {
            if (!asset?.id || !config.editorAssetUsageUrl) return;
            asset.usageCount = Number(asset.usageCount || 0) + 1;
            fetch(`${config.editorAssetUsageUrl}/${encodeURIComponent(asset.id)}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                },
                ...
                console.warn('Editor asset usage tracking failed', error);
            });
        }
        ...
        function insertEditorAsset(asset, options = {}) {
            ...
            if (!isSvgEditorAsset(asset)) {
                const displaySrc = editorAssetDisplayImageSrc(asset);
                const animatedMeta = gifMetadataForSource(asset.src, asset.mime || asset.mimeType || '');
                fabric.Image.fromURL(displaySrc, image => {
                    if (!image) {
                    ...
                    image.set({
                        src: displaySrc,
                        aaOriginalImageSrc: asset.src,
                        aaStaticPreviewSrc: displaySrc !== asset.src ? displaySrc : '',
                        visible: true,
                        dirty: true,
                        ...animatedMeta,
                    });
        ...
        function loadInitialDesign() {
            ...
            if (data && data.renderer === 'fabric') {
                ...
                state.photoboothFrames = normalizePhotoboothFrames(data.photoboothFrames || []);
                state.activePhotoboothFrameIndex = Math.max(0, Math.min(data.activePhotoboothFrameIndex || 0, state.photoboothFrames.length - 1));
                state.guestbook = defaultGuestbookConfig();
                syncGuestbookPanel();
                const shouldOpenPhotobooth = config.canUseGuestMemories === true &&
                    config.hasPhotoboothProjectSurface === true &&
                    (data.editMode === 'photobooth' || state.projectIntent === 'photobooth');

                if (shouldOpenPhotobooth) {
                    state.editMode = 'photobooth';
                    syncEditorToolLimitedMode();
                    loadPageData(state.photoboothFrames[state.activePhotoboothFrameIndex] || createPhotoboothFrameData(1));
                    return;
                }
            ...
            syncEditorToolLimitedMode();
            state.photoboothFrames = normalizePhotoboothFrames([]);
            state.activePhotoboothFrameIndex = 0;
        ...
        function bindEvents() {
            ...
            els.aaEditPhotoboothBtn?.addEventListener('click', () => {
            ...
            els.aaObjectContextMenu?.addEventListener('click', event => {
	                ...
	                event.preventDefault();
	                event.stopPropagation();
	                const contextTarget = state.contextMenuTarget;
	                const contextPointer = state.contextMenuPointer;
	                closeObjectContextMenu();
	                state.contextMenuTarget = contextTarget;
	                state.contextMenuPointer = contextPointer;
	                runObjectContextAction(button.dataset.aaContextAction);
	            });
            els.aaLayerList?.addEventListener('click', aaHandleLayerPanelClick);
            ...
            els.aaMobileInteractionDrawer?.addEventListener('pointerdown', event => event.stopPropagation());
            els.aaMobileInteractionDrawer?.addEventListener('click', event => event.stopPropagation());
            ...
            els.aaFloatingDuplicateBtn?.addEventListener('click', event => {
                event.preventDefault();
                event.stopPropagation();
                const active = getObjectToolbarTarget();
                if (!active) return;
                state.canvas.setActiveObject(active);
                state.canvas.requestRenderAll();
                if (typeof duplicateActive === 'function') duplicateActive();
                requestAnimationFrame(syncObjectFloatingToolbar);
            });
            els.aaFloatingDeleteBtn?.addEventListener('click', event => {
                event.preventDefault();
                event.stopPropagation();
                const active = getObjectToolbarTarget();
                if (!active) return;
                state.canvas.setActiveObject(active);
                state.canvas.requestRenderAll();
                if (typeof deleteActive === 'function') deleteActive();
                hideObjectFloatingToolbar();
            });
            els.aaFloatingInteractionBtn?.addEventListener('click', event => {
                event.preventDefault();
                event.stopPropagation();
                const active = getObjectToolbarTarget();
                if (!active || typeof isMobileInteractionDrawerCandidate !== 'function' ||
                    !isMobileInteractionDrawerCandidate(active) ||
                    typeof openMobileInteractionDrawer !== 'function') {
                    return;
                }
                state.canvas.setActiveObject(active);
                state.canvas.requestRenderAll();
            ...
            els.aaMagicLayerFileInput?.addEventListener('change', async event => {
                try {
                    setMagicLayerUploadPreview(event.target.files?.[0] || null);
                } catch (error) {
                    hideEditorToast();
            ...
            const markImageOutlineDrawerInteraction = (duration = 1800) => {
                state.keepImageOutlineDrawerOpen = true;
                window.clearTimeout(state.keepImageOutlineDrawerTimer);
                state.keepImageOutlineDrawerTimer = window.setTimeout(() => {
                    state.keepImageOutlineDrawerOpen = false;
                    state.keepImageOutlineDrawerTimer = null;
                }, duration);
            };
            els.aaContextImageOutlineBtn?.addEventListener('click', event => {
                ...
                markImageOutlineDrawerInteraction(2400);
            ...
            const readImageOutlineControlValues = () => {
                ...
                const width = Math.max(1, Math.min(60, Math.round(Number(els.aaContextImageOutlineWidthInput?.value) || 1)));
            ...
            const scheduleImageOutlineControls = (delay = 380) => {
                const values = readImageOutlineControlValues();
                if (!values) return;
                markImageOutlineDrawerInteraction(Math.max(2400, delay + 1800));
                window.clearTimeout(state.imageOutlineApplyTimer);
                state.imageOutlineApplyTimer = window.setTimeout(() => {
                    state.imageOutlineApplyTimer = null;
                    applyImageAlphaOutline({
            ...
            const stopOutlinePanelEvent = event => {
                markImageOutlineDrawerInteraction();
                event.stopPropagation();
            };
            [
                ...
                outlinePanel?.addEventListener(eventName, stopOutlinePanelEvent);
                els.aaContextImageOutlineWidthInput?.addEventListener(eventName, stopOutlinePanelEvent);
            });
            ...
            els.aaContextImageFrameBtn?.addEventListener('click', event => {
	            ...
	            document.querySelectorAll('[data-aa-insert-image-frame]').forEach(button => {
	                button.addEventListener('click', event => {
	                    event.preventDefault();
	                    addImageFramePlaceholder(button.dataset.aaInsertImageFrame);
	                });
	            });
            ...
            els.aaTextContextColorInput?.addEventListener('input', event => {
                const target = getContextTextTarget();
                if (!target) return;
                if (typeof aaClearMaterialMetadata === 'function') aaClearMaterialMetadata(target);
                applyActiveStyle({
                    fill: event.target.value
                });
                if (els.aaColorInput) {
            ...
            const closeLeftDrawer = () => {
                if (typeof noteMobileInteractionDrawerClosed === 'function') {
                    noteMobileInteractionDrawerClosed();
                }
                closeLeftDrawerPanel();
            };
            ...
            document.querySelectorAll('[data-aa-left-tab]').forEach(tab => {
                tab.addEventListener('click', event => {
                    ...
                    if (document.body.classList.contains('aa-editor-mobile-mode')) {
                        document.body.classList.remove('aa-mobile-tools-open');
                        document.getElementById('aaMobileToolsBtn')?.setAttribute('aria-expanded', 'false');
                    }
                });
            });
            ...
            bindFontDrawerTrigger(els.aaMobileGuestFieldFontInput, 'guest-field');
            bindFontDrawerTrigger(els.aaMobileOpeningButtonFontInput, 'opening-button');
            ...
            els.aaFontDrawerCloseBtn?.addEventListener('click', event => {
                event.preventDefault();
                if (typeof returnToMobileInteractionDrawer === 'function' && returnToMobileInteractionDrawer('font')) return;
                closeLeftDrawerPanel();
            });
            els.aaColorDrawerCloseBtn?.addEventListener('click', event => {
                event.preventDefault();
                if (typeof returnToMobileInteractionDrawer === 'function' && returnToMobileInteractionDrawer('color')) return;
                closeLeftDrawerPanel();
            });
            ...
            document.querySelectorAll('[data-aa-material-preset]').forEach(button => {
                button.addEventListener('click', event => {
                    event.preventDefault();
                    const target = aaGetMaterialTarget();
                    if (!target) {
                        setStatus?.('Foil dan glitter khusus untuk teks');
                        return;
                    }
                    if (aaApplyMaterialToObject(target, button.dataset.aaMaterialPreset, true)) {
                        setStatus?.('Material warna diterapkan');
                    }
                });
            });
            ...
            els.aaTemplateSearchInput?.addEventListener('keydown', event => {
                if (event.key !== 'Enter') return;
                event.preventDefault();
                filterTemplateDrawer();
            });
            els.aaTemplateSearchBtn?.addEventListener('click', filterTemplateDrawer);
            ...
            els.aaTemplateCategoryChips?.addEventListener('scroll', updateTemplateChipScrollState, {
                passive: true
            });
            document.querySelectorAll('[data-aa-template-chip-scroll]').forEach(button => {
                button.addEventListener('click', () => {
                    scrollTemplateCategoryChips(Number(button.dataset.aaTemplateChipScroll || 1));
                });
            });
            ...
            showTemplateCategoryView();
            window.addEventListener('resize', updateTemplateChipScrollState, {
                passive: true
            });
            document.querySelectorAll('[data-aa-template-preview]').forEach(button => {
                button.addEventListener('click', event => {
                    event.preventDefault();
                    event.stopPropagation();
                    const more = event.target.closest?.('[data-aa-template-more]');
                    if (more) {
                        openTemplateDetailPopover(button, more);
                        return;
                    }
            ...
            els.aaTemplateDetailPopover?.addEventListener('click', event => {
                const previewButton = event.target.closest('[data-aa-template-detail-preview]');
                if (!previewButton) return;
                event.preventDefault();
                const card = els.aaTemplateDetailPopover.__aaTemplateCard;
                closeTemplateDetailPopover();
                if (card) {
                    openTemplatePreview(card.dataset.aaTemplatePreview, card.dataset.aaTemplateTitle || 'Template', card
                        .dataset.aaTemplatePublicUrl || '');
                }
            });
            ...
            document.addEventListener('click', event => {
                if (!event.target.closest?.('[data-aa-template-more], #aaTemplateDetailPopover')) {
                    closeTemplateDetailPopover();
                }
            ...
            els.aaEditorAssetGlobalSearchInput?.addEventListener('keydown', event => {
                if (event.key !== 'Enter') return;
                event.preventDefault();
                showEditorAssetGlobalResults(event.target.value || '');
            });
            els.aaEditorAssetGlobalSearchBtn?.addEventListener('click', () => {
                showEditorAssetGlobalResults(els.aaEditorAssetGlobalSearchInput?.value || '');
            });
            els.aaEditorAssetQuickChips?.addEventListener('click', event => {
                const button = event.target.closest('[data-aa-editor-asset-query]');
                if (!button) return;
                showEditorAssetGlobalResults(button.dataset.aaEditorAssetQuery || '', button.textContent.trim());
            });
            els.aaEditorAssetQuickChips?.addEventListener('scroll', updateEditorAssetQuickScrollState, {
                passive: true
            });
            document.querySelectorAll('[data-aa-editor-asset-quick-scroll]').forEach(button => {
                button.addEventListener('click', () => {
                    scrollEditorAssetQuickChips(Number(button.dataset.aaEditorAssetQuickScroll || 1));
                });
            });
            els.aaEditorAssetLandingSections?.addEventListener('click', event => {
                const openType = event.target.closest('[data-aa-editor-asset-open-type]');
                if (openType) {
                    openEditorAssetTypeSection(openType.dataset.aaEditorAssetOpenType || 'all', openType
                        .dataset.aaEditorAssetOpenLabel || openType.textContent.trim());
                    return;
                }
                const openCategory = event.target.closest('[data-aa-editor-asset-category]');
                if (openCategory) {
                    event.preventDefault();
                    showEditorAssetListView(openCategory.dataset.aaEditorAssetCategory, openCategory.dataset
                        .aaEditorAssetCategoryLabel || openCategory.textContent.trim());
                    return;
                }
                const frameButton = event.target.closest('[data-aa-frame-preset]');
                if (frameButton) {
                    event.preventDefault();
                    insertPhotoFramePreset(frameButton.dataset.aaFramePreset);
                    return;
                }
                ...
                if (asset) insertEditorAsset(asset);
            });
            ...
            els.aaEditorAssetTypeChips?.addEventListener('click', event => {
                const button = event.target.closest('[data-aa-editor-asset-type]');
                if (!button) return;
                state.editorAssetType = button.dataset.aaEditorAssetType || 'all';
                state.editorAssetVisible = 40;
                renderEditorAssetTypeChips();
                renderEditorAssets();
            });
            els.aaEditorAssetTypeChips?.addEventListener('scroll', updateEditorAssetTypeScrollState, {
                passive: true
            });
            document.querySelectorAll('[data-aa-editor-asset-type-scroll]').forEach(button => {
                button.addEventListener('click', () => {
                    scrollEditorAssetTypeChips(Number(button.dataset.aaEditorAssetTypeScroll || 1));
                });
            });
            window.addEventListener('resize', updateEditorAssetTypeScrollState, {
                passive: true
            });
            window.addEventListener('resize', updateEditorAssetQuickScrollState, {
                passive: true
            });
            ...
            els.aaEditorAssetGrid?.addEventListener('click', event => {
                ...
                const menuButton = event.target.closest('[data-aa-editor-asset-menu-id]');
                if (menuButton) {
                    event.preventDefault();
                    event.stopPropagation();
                    const id = menuButton.dataset.aaEditorAssetMenuId || '';
                    state.editorAssetMenuId = String(state.editorAssetMenuId || '') === String(id) ? null : id;
                    renderEditorAssets();
                    return;
                }

                const saveCategoryButton = event.target.closest('[data-aa-editor-asset-category-save]');
                if (saveCategoryButton) {
                    event.preventDefault();
                    event.stopPropagation();
                    const id = saveCategoryButton.dataset.aaEditorAssetCategorySave || '';
                    const typeSelect = els.aaEditorAssetGrid.querySelector(`[data-aa-editor-asset-type-select="${CSS.escape(id)}"]`);
                    const select = els.aaEditorAssetGrid.querySelector(`[data-aa-editor-asset-category-select="${CSS.escape(id)}"]`);
                    const premiumSelect = els.aaEditorAssetGrid.querySelector(`[data-aa-editor-asset-premium-select="${CSS.escape(id)}"]`);
                    updateEditorAssetCategory(id, {
                        type: typeSelect?.value || 'ornament',
                        category: select?.value || '',
                        isPremium: premiumSelect?.checked === true,
                    });
                    return;
                }

                if (event.target.closest('[data-aa-editor-asset-menu-close]')) {
                    event.preventDefault();
                    event.stopPropagation();
                    state.editorAssetMenuId = null;
                    renderEditorAssets();
                ...
                if (event.target.closest('[data-aa-editor-asset-menu-panel]')) {
                    event.stopPropagation();
                    return;
                }
            ...
            document.addEventListener('click', event => {
                if (!state.editorAssetMenuId) return;
                const target = event.target instanceof Element ? event.target : null;
                if (target?.closest('.aa-editor-asset-menu-panel, .aa-editor-asset-menu-toggle')) return;
                state.editorAssetMenuId = null;
                renderEditorAssets();
            });
            ...
            document.querySelectorAll('[data-aa-animation]').forEach(button => {
	            ...
	            const updateActiveAnimateTiming = (commit, input) => {
	                ...
	                if (textTarget && aaGetTextAnimationConfig(textTarget).enabled) {
	                    if (typeof updateActiveTextAnimationTimingFromPanel === 'function' &&
	                        updateActiveTextAnimationTimingFromPanel(commit, input) !== false) {
	                        return;
	                    }
            ...
            document.querySelectorAll('[data-aa-text-animation]').forEach(button => {
	            ...
	            const updateActiveTextAnimateTiming = (commit, input) => {
	                if (typeof updateActiveTextAnimationTimingFromPanel === 'function' &&
	                    updateActiveTextAnimationTimingFromPanel(commit, input) !== false) {
	                    return;
	                }
	            ...
	            document.querySelectorAll('[data-aa-text-animation-stagger]').forEach(input => {
	                input.addEventListener('input', event => updateActiveTextAnimateTiming(false, event.currentTarget));
	                input.addEventListener('change', event => updateActiveTextAnimateTiming(true, event.currentTarget));
	            });

	            $('aaUndoBtn').addEventListener('click', undo);
	            $('aaRedoBtn').addEventListener('click', redo);
	            $('aaZoomOutBtn').addEventListener('click', () => {
	                setEditorZoom(state.zoom - 0.08, { manual: true });
	            });
		            $('aaZoomInBtn').addEventListener('click', () => {
		                setEditorZoom(state.zoom + 0.08, { manual: true });
		            });
	            $('aaFitBtn').addEventListener('click', fitZoom);
	            document.getElementById('aaMobileSaveBtn')?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                document.getElementById('aaSaveBtn')?.click();
	            });
            ...
            els.aaLinkPopoverUrlInput?.addEventListener('input', event => applyLinkInteractionValue(event.target
                .value));
            els.aaMobileLinkUrlInput?.addEventListener('input', event => applyLinkInteractionValue(event.target
                .value));
            ...
            els.aaMobileSocialPlatformInput?.addEventListener('change', event => {
                const platform = event.target.value || 'instagram';
                const meta = aaSocialPlatformMeta(platform);
                applySocialLinkValue({
                    socialPlatform: platform,
            ...
            els.aaMobileSocialLabelInput?.addEventListener('input', event => applySocialLinkValue({
                socialLabel: event.target.value,
            }));
            els.aaSocialPopoverUrlInput?.addEventListener('input', event => applySocialLinkValue({
                link: event.target.value,
            }));
            els.aaMobileSocialUrlInput?.addEventListener('input', event => applySocialLinkValue({
                link: event.target.value,
            }));
            ...
            els.aaMobileCopyTextInput?.addEventListener('input', event => applyCopyInteractionValue({
                copyText: event.target.value,
            }));
            ...
            els.aaMobileCopyFeedbackInput?.addEventListener('input', event => applyCopyInteractionValue({
                copyFeedback: event.target.value,
            }));
            ...
            els.aaMobileYoutubeUrlInput?.addEventListener('input', event => applyYoutubeInteractionValue({
                youtubeUrl: event.target.value,
            }));
            ...
            els.aaMobileYoutubeBgInput?.addEventListener('input', event => applyYoutubePopoverStyle({
                controlBackground: event.target.value || '#111827',
            }));
            els.aaYoutubePopoverRadiusInput?.addEventListener('input', event => applyYoutubePopoverStyle({
                controlRadius: event.target.value,
            }));
            els.aaMobileYoutubeRadiusInput?.addEventListener('input', event => {
                if (els.aaMobileYoutubeRadiusValue) els.aaMobileYoutubeRadiusValue.textContent = Math.max(0, Math.round(Number(event.target.value) || 0));
                applyYoutubePopoverStyle({
                    controlRadius: event.target.value,
                });
            });
            ...
            els.aaMobileYoutubeAutoplayInput?.addEventListener('change', event => applyYoutubeInteractionValue({
                youtubeAutoplayOnView: event.target.checked,
            }));
            els.aaYoutubePopoverLoopInput?.addEventListener('change', event => applyYoutubeInteractionValue({
                youtubeLoop: event.target.checked,
            }));
            els.aaMobileYoutubeLoopInput?.addEventListener('change', event => applyYoutubeInteractionValue({
                youtubeLoop: event.target.checked,
            }));
            ...
            els.aaMobileOpeningButtonBgInput?.addEventListener('input', event => applyOpeningButtonInteractionValue({
                controlBackground: event.target.value,
            }));
            ...
            els.aaMobileOpeningButtonTextColorInput?.addEventListener('input', event => applyOpeningButtonInteractionValue({
                controlTextColor: event.target.value,
            }));
            ...
            els.aaMobileOpeningButtonFontInput?.addEventListener('change', event => {
                const fontFamily = event.target.value || 'Inter';
                loadEditorFontFamily(fontFamily, loadedFamily => applyOpeningButtonInteractionValue({
                    fontFamily: loadedFamily,
                }));
            ...
            els.aaMobileOpeningButtonRadiusInput?.addEventListener('input', event => {
                if (els.aaMobileOpeningButtonRadiusValue) els.aaMobileOpeningButtonRadiusValue.textContent = Math.max(0, Math.round(Number(event.target.value) || 0));
                applyOpeningButtonInteractionValue({
                    controlRadius: event.target.value,
                });
            });
            ...
            els.aaMobileOpeningButtonPaddingYInput?.addEventListener('input', event => {
                if (els.aaMobileOpeningButtonPaddingYValue) els.aaMobileOpeningButtonPaddingYValue.textContent = Math.max(0, Math.round(Number(event.target.value) || 0));
                applyOpeningButtonInteractionValue({
                    padding: event.target.value,
                });
            });
            ...
            els.aaMobileGuestFieldTextInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                text: event.target.value,
            }));
            els.aaGuestFieldPopoverBgInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                backgroundColor: event.target.value,
            }));
            els.aaMobileGuestFieldBgInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                backgroundColor: event.target.value,
            }));
            ...
            els.aaMobileGuestFieldFontInput?.addEventListener('change', event => {
                const fontFamily = event.target.value || 'Inter';
                const apply = () => applyGuestFieldInteractionValue({
                    fontFamily,
                });
            ...
            els.aaMobileGuestFieldSizeInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                fontSize: event.target.value,
            }));
            els.aaGuestFieldPopoverColorInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                fill: event.target.value,
            }));
            els.aaMobileGuestFieldColorInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                fill: event.target.value,
            }));
            els.aaGuestFieldPopoverRadiusInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                borderRadius: event.target.value,
            }));
            els.aaMobileGuestFieldRadiusInput?.addEventListener('input', event => {
                if (els.aaMobileGuestFieldRadiusValue) els.aaMobileGuestFieldRadiusValue.textContent = Math.max(0, Math.round(Number(event.target.value) || 0));
            ...
            els.aaMobileGuestFieldRequiredInput?.addEventListener('change', event =>
                applyGuestFieldInteractionValue({
                    required: event.target.checked,
                }));
            ...
            els.aaMobileGuestFieldMaxInput?.addEventListener('input', event => applyGuestFieldInteractionValue({
                maxLength: event.target.value,
            }));
            ...
            els.aaMobileGalleryUploadBtn?.addEventListener('click', event => {
                if (guardPremiumFeature(event)) return;
                const active = typeof getInteractionApplyTarget === 'function'
                    ? getInteractionApplyTarget(isGalleryInteractionObject)
                    : activeGalleryObject();
                if (!active || active.customType !== 'photo-gallery') {
                    setStatus('Pilih elemen gallery terlebih dahulu.', 'error');
                    return;
                }
                state.canvas.setActiveObject(active);
                ...
                els.aaImageInput.click();
		            ...
		            els.aaMobileGalleryPickMediaBtn?.addEventListener('click', event => {
		                if (guardPremiumFeature(event)) return;
		                if (guardMediaLibraryFeature(event)) return;
		                const active = typeof getInteractionApplyTarget === 'function'
		                    ? getInteractionApplyTarget(isGalleryInteractionObject)
	                ...
	                state.canvas.setActiveObject(active);
		                state.mediaMode = 'gallery';
		                openMobileMediaLibraryForPick('Pilih foto dari Media Library.');
		            });
	            els.aaMobileGalleryPhotoUploadBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (!beginGalleryPhotoReplaceMode()) return;
	                els.aaImageInput.click();
	            });
		            els.aaMobileGalleryPhotoPickMediaBtn?.addEventListener('click', event => {
		                event.preventDefault();
		                event.stopPropagation();
		                if (guardMediaLibraryFeature(event)) return;
		                if (!beginGalleryPhotoReplaceMode()) return;
		                openMobileMediaLibraryForPick('Pilih foto pengganti dari Media Library.');
		            });
	            els.aaMobileGalleryPhotoZoomInput?.addEventListener('change', event => {
	                const active = typeof getInteractionApplyTarget === 'function'
	                    ? getInteractionApplyTarget(isGalleryPhotoInteractionObject)
	                    : state.canvas.getActiveObject();
	                if (!isGalleryPhotoObject(active)) return;
	                const enabled = event.target.checked === true;
	                active.set({
	                    customType: 'gallery-photo',
	                    isGalleryPhoto: true,
	                    galleryZoom: enabled,
	                    galleryImageSrc: enabled ? (readImageSourceUrl(active) || active.galleryImageSrc || '') : '',
	                });
	                active.setCoords();
	                state.canvas.requestRenderAll();
	                syncInspector();
	                snapshot();
	                setStatus(enabled ? 'Zoom gallery saat publish aktif' : 'Zoom gallery saat publish nonaktif');
	            });
	            els.aaMobileImageUploadBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (!beginMobileImageReplaceMode()) return;
	                els.aaImageInput.click();
	            });
		            els.aaMobileImagePickMediaBtn?.addEventListener('click', event => {
		                event.preventDefault();
		                event.stopPropagation();
		                if (guardMediaLibraryFeature(event)) return;
		                if (!beginMobileImageReplaceMode()) return;
		                openMobileMediaLibraryForPick('Pilih gambar pengganti dari Media Library.');
		            });
	            els.aaMobileImageRadiusInput?.addEventListener('input', event => {
	                const active = typeof getInteractionApplyTarget === 'function'
	                    ? getInteractionApplyTarget(isMobileImageInteractionObject)
	                    : state.canvas.getActiveObject();
	                if (!isMobileReplaceableImageObject(active)) return;
	                const radius = Math.max(0, Number(event.target.value) || 0);
	                if (els.aaMobileImageRadiusValue) els.aaMobileImageRadiusValue.textContent = Math.round(radius);
	                if (els.aaImageRadiusInput) els.aaImageRadiusInput.value = radius;
	                if (els.aaImageRadiusValue) els.aaImageRadiusValue.textContent = `${Math.round(radius)}px`;
	                applyImageBorderRadius(active, radius);
	                state.canvas.requestRenderAll();
	                syncContextRadiusControl(active);
	            });
	            els.aaMobileImageRadiusInput?.addEventListener('change', () => {
	                const active = typeof getInteractionApplyTarget === 'function'
	                    ? getInteractionApplyTarget(isMobileImageInteractionObject)
		                    : state.canvas.getActiveObject();
	                if (!isMobileReplaceableImageObject(active)) return;
	                storeCurrentPage();
		                snapshot();
		                setStatus('Radius gambar diperbarui');
		            });
	            els.aaMobileFrameUploadBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (!beginMobileFramePhotoFillMode()) return;
	                els.aaImageInput.click();
	            });
	            els.aaMobileFramePickMediaBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (guardMediaLibraryFeature(event)) return;
	                if (!beginMobileFramePhotoFillMode()) return;
	                openMobileMediaLibraryForPick('Pilih foto untuk Frame Photo.');
	            });
	            document.querySelectorAll('[data-aa-mobile-frame-shape]').forEach(button => {
	                button.addEventListener('click', event => {
	                    event.preventDefault();
	                    applyMobileFramePhotoShape(button.dataset.aaMobileFrameShape || 'rounded');
	                });
	            });
	            els.aaFramePlaceholderUploadBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (!beginFramePlaceholderFillMode()) return;
	                els.aaImageInput.click();
	            });
	            els.aaFramePlaceholderPickMediaBtn?.addEventListener('click', event => {
	                event.preventDefault();
	                event.stopPropagation();
	                if (!beginFramePlaceholderFillMode()) return;
	                if (window.matchMedia('(max-width: 767px)').matches) {
	                    if (guardMediaLibraryFeature(event)) return;
		                    openMobileMediaLibraryForPick('Pilih foto dari Media Library untuk mengisi frame.');
		                    return;
		                }
		                if (typeof openLeftDrawerPanel === 'function') openLeftDrawerPanel('upload');
		                if (typeof loadMedia === 'function') loadMedia({ force: true }).catch(() => {});
		                hidePendingEditorToast();
		                setStatus('Pilih foto dari Media Library untuk mengisi frame.');
		            });
            ...
            [els.aaMobileGalleryColumnsInput, els.aaMobileGalleryGapInput, els.aaMobileGalleryRadiusInput]
            .forEach(input => {
                input?.addEventListener('input', event => {
                    const active = typeof getInteractionApplyTarget === 'function'
                        ? getInteractionApplyTarget(isGalleryInteractionObject)
                        : activeGalleryObject();
                    if (!active || active.customType !== 'photo-gallery') return;
                    const columns = Math.max(1, Math.min(6, Number(els.aaMobileGalleryColumnsInput?.value) || 2));
                    const gap = Math.max(0, Number(els.aaMobileGalleryGapInput?.value) || 0);
                    const radius = Math.max(0, Number(els.aaMobileGalleryRadiusInput?.value) || 0);
                    const items = normalizeGalleryItems(active);
                    active.set({
                        galleryItems: items,
                        galleryImages: items.map(item => item.src),
                    ...
                    if (els.aaGalleryColumnsInput) els.aaGalleryColumnsInput.value = columns;
                    if (els.aaGalleryGapInput) els.aaGalleryGapInput.value = gap;
                    if (els.aaGalleryRadiusInput) els.aaGalleryRadiusInput.value = radius;
                    refreshInteractivePreviewObject(active);
                });
            });
            ...
            document.querySelectorAll('[data-aa-tablet-action]').forEach(button => {
                button.addEventListener('click', event => {
                    event.preventDefault();
                    const targetId = {
                        preview: 'aaPreviewBtn',
                        save: 'aaSaveBtn',
                        'save-template': 'saveTemplateBtn',
                        publish: 'aaPublishBtn',
                    }[button.dataset.aaTabletAction || ''];
            ...
            function aaPreparePublishFields() {
                ...
                if (els.aaPublishSubdomainInput && !els.aaPublishSubdomainInput.value) {
                    els.aaPublishSubdomainInput.value = els.aaPublishSlugInput.value;
                }
            }

            function aaShowPublishChoiceView() {
                els.aaPublishChoiceView?.classList.remove('hidden');
                els.aaPublishDetailView?.classList.add('hidden');
            }

            function aaShowPublishDetailView() {
                aaPreparePublishFields();
                checkSlug().catch(() => {});
                resetPublishOgPreview();
                els.aaPublishChoiceView?.classList.add('hidden');
                els.aaPublishDetailView?.classList.remove('hidden');
            }

            function aaClosePublishModal() {
            ...
            $('aaPublishBtn')?.addEventListener('click', event => {
                ...
                hideCountdownContextToolbar();
                hideInteractionPopover();
                aaShowPublishChoiceView();
                els.aaPublishModal.classList.add('is-open');
            });
            els.aaPublishChoiceInvitationBtn?.addEventListener('click', aaShowPublishDetailView);
            document.getElementById('aaPublishChoiceBusinessProfileBtn')?.addEventListener('click', event => {
                const checkoutUrl = String(event.currentTarget?.dataset?.businessProfileCheckoutUrl || '');
                if (checkoutUrl) {
                    window.location.href = checkoutUrl;
                    return;
                }
                aaShowPublishDetailView();
            });
            els.aaPublishBackBtn?.addEventListener('click', aaShowPublishChoiceView);
            els.aaClosePublishChoiceBtn?.addEventListener('click', aaClosePublishModal);
            $('aaClosePublishBtn').addEventListener('click', aaClosePublishModal);
            let aaPublishSubdomainManuallyEdited = (() => {
                const slug = normalizeSlug(els.aaPublishSlugInput?.value || config.initialSlug || config.initialTitle);
                const subdomain = normalizePublishSubdomain(els.aaPublishSubdomainInput?.value || '');
                return subdomain !== '' && slug !== '' && subdomain !== slug;
            })();
            function aaSyncPublishSubdomainFromSlug(slugValue) {
                if (!els.aaPublishSubdomainInput) return;
                const slug = normalizeSlug(slugValue || '');
                if (!slug) return;
                const current = normalizePublishSubdomain(els.aaPublishSubdomainInput.value || '');
                if (!aaPublishSubdomainManuallyEdited || current === '') {
                    els.aaPublishSubdomainInput.value = slug;
                }
            }
            els.aaPublishSlugInput.addEventListener('input', () => {
                aaSyncPublishSubdomainFromSlug(els.aaPublishSlugInput.value);
                clearTimeout(state.slugTimer);
                state.slugTimer = setTimeout(() => checkSlug().catch(() => {}), 350);
            });
            els.aaPublishSubdomainInput?.addEventListener('input', () => {
                const slug = normalizeSlug(els.aaPublishSlugInput?.value || config.initialSlug || config.initialTitle);
                const subdomain = normalizePublishSubdomain(els.aaPublishSubdomainInput.value || '');
                aaPublishSubdomainManuallyEdited = subdomain !== '' && subdomain !== slug;
                clearTimeout(state.slugTimer);
                state.slugTimer = setTimeout(() => checkSlug().catch(() => {}), 250);
            });
            document.querySelectorAll('input[name="aa_publish_root_domain"]').forEach(input => {
                input.addEventListener('change', () => checkSlug().catch(() => {}));
            });
            els.aaPublishOgImageInput?.addEventListener('change', () => {
                refreshPublishOgPreview(els.aaPublishOgImageInput.files?.[0] || null);
            });
            els.aaPublishOgClearBtn?.addEventListener('click', () => {
                resetPublishOgPreview();
            });

            let aaPhotoboothDomainStandardUrl = '';
            let aaPhotoboothDomainPayload = null;
            let aaPhotoboothPublishIntent = false;

            function aaPhotoboothDomainRupiah(value) {
                return 'Rp' + Number(value || 0).toLocaleString('id-ID') + ' / tahun';
            }

            function aaSetPhotoboothDomainMessage(message, tone = '') {
                if (!els.aaPhotoboothDomainMessage) return;
                els.aaPhotoboothDomainMessage.textContent = message || '';
                els.aaPhotoboothDomainMessage.classList.toggle('text-rose-600', tone === 'error');
                els.aaPhotoboothDomainMessage.classList.toggle('text-emerald-600', tone === 'success');
                els.aaPhotoboothDomainMessage.classList.toggle('text-slate-500', tone !== 'error' && tone !== 'success');
            }

            function aaSetPhotoboothDomainMode(mode) {
                const input = els.aaPhotoboothDomainForm?.querySelector(`input[name="domain_mode"][value="${mode}"]`);
                if (input) input.checked = true;
            }

            function aaGetPhotoboothPublicDomainUrl(payload) {
                const domain = String(payload?.domain || '').replace(/^https?:\/\//i, '').replace(/\/+$/g, '').trim();
                return domain ? `https://${domain}` : '';
            }

            function aaGetInvitationPublicUrlForPhotobooth() {
                const slug = normalizeSlug(els.aaPublishSlugInput?.value || config.initialSlug || config.initialTitle);
                if (els.aaPublishSlugInput && slug) {
                    els.aaPublishSlugInput.value = slug;
                }
                return slug ? `${config.publicBaseUrl}${slug}` : '';
            }

            function aaSyncPhotoboothInvitationPublicLink() {
                const invitationUrl = aaGetInvitationPublicUrlForPhotobooth();
                if (els.aaPhotoboothReadyUrlInput) {
                    els.aaPhotoboothReadyUrlInput.value = invitationUrl || 'Link undangan belum tersedia';
                }
                if (!els.aaPhotoboothReadyUrlStatus) return;
                els.aaPhotoboothReadyUrlStatus.textContent = 'Link undangan utama siap dibagikan.';
                els.aaPhotoboothReadyUrlStatus.classList.add('is-active');
                els.aaPhotoboothReadyUrlStatus.classList.remove('is-pending');
            }

            function aaSyncPhotoboothLinkFields(payload) {
                const standardUrl = String(payload?.standard_url || aaPhotoboothDomainStandardUrl || '').trim();
                const publicUrl = aaGetPhotoboothPublicDomainUrl(payload);
                const status = String(payload?.status || '');
                const standardUrlLabel = els.aaPhotoboothStandardUrl?.querySelector('span') || els.aaPhotoboothStandardUrl;
                if (standardUrlLabel) {
                    standardUrlLabel.textContent = standardUrl || 'Link photobooth belum tersedia.';
                }
                if (els.aaPhotoboothPreviewUrlInput) {
                    els.aaPhotoboothPreviewUrlInput.value = standardUrl;
                }
                aaSyncPhotoboothInvitationPublicLink();
                if (els.aaPhotoboothPublicUrlInput) {
                    els.aaPhotoboothPublicUrlInput.value = publicUrl || 'Belum ada custom domain';
                }
                if (els.aaPhotoboothPublicUrlCopyBtn) {
                    els.aaPhotoboothPublicUrlCopyBtn.disabled = publicUrl === '';
                    els.aaPhotoboothPublicUrlCopyBtn.classList.toggle('opacity-50', publicUrl === '');
                    els.aaPhotoboothPublicUrlCopyBtn.classList.toggle('cursor-not-allowed', publicUrl === '');
                }
                if (els.aaPhotoboothPublicUrlStatus) {
                    els.aaPhotoboothPublicUrlStatus.textContent = publicUrl
                        ? (status === 'active'
                            ? 'Custom domain aktif dan siap dibagikan.'
                            : 'Custom domain tersimpan. Link ini bisa digunakan setelah status aktif.')
                        : 'Gunakan custom domain jika ingin public link khusus untuk Photobooth.';
                    els.aaPhotoboothPublicUrlStatus.classList.toggle('is-active', publicUrl !== '' && status === 'active');
                    els.aaPhotoboothPublicUrlStatus.classList.toggle('is-pending', publicUrl === '' || status !== 'active');
                }
            }

            function aaIsPhotoboothDomainLocked(payload) {
                const status = String(payload?.status || '');
                return payload?.mode === 'custom' && payload?.domain && status !== 'unavailable';
            }

            function aaCanPublishPhotoboothDomain(payload) {
                if (!aaPhotoboothPublishIntent) return true;
                if (!aaIsPhotoboothDomainLocked(payload)) return true;
                return String(payload?.status || '') === 'active';
            }

            function aaSetPhotoboothDomainLocked(payload) {
                const locked = aaIsPhotoboothDomainLocked(payload);
                const canPublishDomain = aaCanPublishPhotoboothDomain(payload);
                const domain = String(payload?.domain || '');
                const statusHeader = els.aaPhotoboothDomainStatusPanel?.querySelector('.aa-photobooth-status-head');
                const statusTextBlock = statusHeader?.firstElementChild;
                els.aaPhotoboothDomainOptions?.classList.toggle('hidden', locked);
                els.aaPhotoboothDomainSubmitBtn?.classList.toggle('hidden', locked && !aaPhotoboothPublishIntent);
                if (els.aaPhotoboothDomainSubmitBtn) {
                    const submitText = aaPhotoboothPublishIntent
                        ? (locked ? 'Publish Frame Photobooth' : 'Simpan & Publish Frame')
                        : 'Simpan Pilihan';
                    if (!els.aaPhotoboothDomainSubmitBtn.classList.contains('is-loading')) {
                        delete els.aaPhotoboothDomainSubmitBtn.dataset.originalHtml;
                        els.aaPhotoboothDomainSubmitBtn.textContent = submitText;
                    }
                    els.aaPhotoboothDomainSubmitBtn.disabled = !canPublishDomain;
                    els.aaPhotoboothDomainSubmitBtn.classList.toggle('opacity-50', !canPublishDomain);
                    els.aaPhotoboothDomainSubmitBtn.classList.toggle('cursor-not-allowed', !canPublishDomain);
                    els.aaPhotoboothDomainSubmitBtn.title = !canPublishDomain
                        ? 'Frame Photobooth custom domain bisa dipublish setelah domain aktif.'
                        : '';
                }
                els.aaPhotoboothDomainPrice?.classList.toggle('hidden', locked);
                els.aaPhotoboothDomainStatusPanel?.classList.toggle('text-center', locked);
                els.aaPhotoboothDomainStatusPanel?.classList.toggle('py-8', locked);
                ...
                if (els.aaPhotoboothDomainValue) {
                    els.aaPhotoboothDomainValue.textContent = domain;
                    els.aaPhotoboothDomainValue.classList.toggle('hidden', !locked || domain === '');
                }
            }

            function aaApplyPhotoboothDomainPayload(payload) {
                if (!payload) return;
                aaPhotoboothDomainPayload = payload;
                if (payload.csrf_hash) config.csrfHash = payload.csrf_hash;
                aaPhotoboothDomainStandardUrl = payload.standard_url || '';
                aaSyncPhotoboothLinkFields(payload);
                if (els.aaPhotoboothDomainStatus) {
                    els.aaPhotoboothDomainStatus.textContent = payload.status_label || 'Domain sedang dicek';
                }
                if (els.aaPhotoboothDomainPrice) {
                    els.aaPhotoboothDomainPrice.textContent = aaPhotoboothDomainRupiah(payload.price || 250000);
                }
                if (els.aaPhotoboothDomainNote) {
                    els.aaPhotoboothDomainNote.textContent = payload.notes || 'Nama domain yang dipilih akan dicek ketersediaannya oleh admin. Setelah tersedia dan pembayaran dikonfirmasi, domain akan disiapkan dan dihubungkan ke Photobooth.';
                }
                if (els.aaPhotoboothCustomDomainInput) {
                    els.aaPhotoboothCustomDomainInput.value = payload.domain || '';
                }
                aaSetPhotoboothDomainMode(payload.mode === 'custom' ? 'custom' : 'adaacara');
                aaSetPhotoboothDomainLocked(payload);
                aaSyncPhotoboothPaymentPanel(payload);
            }

            function aaSyncPhotoboothPaymentPanel(payload) {
                if (!els.aaPhotoboothDomainPaymentPanel) return;
                ...
                els.aaPhotoboothDomainPaymentPanel.classList.toggle('hidden', !shouldShow);
                if (els.aaPhotoboothPaymentInstruction) {
                    els.aaPhotoboothPaymentInstruction.textContent = payload?.payment_instruction || 'Transfer add-on custom domain Photobooth sebesar Rp250.000/tahun. Setelah transfer, upload bukti pembayaran di sini agar admin dapat mengonfirmasi dan menyiapkan aktivasi domain.';
                }
                const paymentOrder = payload?.payment_order || null;
                if (els.aaPhotoboothPaymentOrderStatus) {
                    const orderLabel = paymentOrder?.invoice_number
                        ...
                        : '';
                    els.aaPhotoboothPaymentOrderStatus.textContent = orderLabel;
                    els.aaPhotoboothPaymentOrderStatus.classList.toggle('hidden', orderLabel === '');
                }
                const checkoutUrl = String(payload?.payment_checkout_url || '');
                if (els.aaPhotoboothPaymentCheckoutBtn) {
                    els.aaPhotoboothPaymentCheckoutBtn.href = checkoutUrl || '#';
                    els.aaPhotoboothPaymentCheckoutBtn.textContent = paymentOrder?.detail_url ? 'Lihat Invoice' : 'Lanjut Pembayaran';
                    els.aaPhotoboothPaymentCheckoutBtn.classList.toggle('hidden', !canUpload || checkoutUrl === '');
                }
                const uploadControlsHidden = !canUpload;
                els.aaPhotoboothPaymentProofInput?.classList.toggle('hidden', uploadControlsHidden);
                els.aaPhotoboothPaymentNoteInput?.classList.toggle('hidden', uploadControlsHidden);
                els.aaPhotoboothPaymentUploadBtn?.classList.toggle('hidden', uploadControlsHidden);
                if (els.aaPhotoboothPaymentProofStatus) {
                    els.aaPhotoboothPaymentProofStatus.textContent = paymentStatus === 'paid'
                        ? 'Pembayaran sudah dikonfirmasi admin.'
                        : (hasSubmitted ? 'Bukti pembayaran sudah terkirim dan menunggu konfirmasi admin.' : '');
                }
            }

            async function aaRequestPhotoboothDomain(url, options = {}) {
                ...
                if (!response.ok || payload.success === false || payload.status === false) {
                    throw new Error(payload.message || 'Request domain Photobooth gagal.');
                }
                return payload;
            }

            async function aaOpenPhotoboothDomainModal(options = {}) {
                if (!els.aaPhotoboothDomainModal || !config.photoboothDomainUrl) return;
                aaPreparePublishFields();
                aaPhotoboothPublishIntent = options.publishIntent === true;
                aaPhotoboothDomainPayload = null;
                if (els.aaPhotoboothDomainTitle) {
                    els.aaPhotoboothDomainTitle.textContent = els.aaPublishTitleInput?.value || config.initialTitle || 'Undangan';
                }
                const standardUrlLabel = els.aaPhotoboothStandardUrl?.querySelector('span') || els.aaPhotoboothStandardUrl;
                if (standardUrlLabel) standardUrlLabel.textContent = 'Memuat...';
                if (els.aaPhotoboothPreviewUrlInput) els.aaPhotoboothPreviewUrlInput.value = '';
                if (els.aaPhotoboothReadyUrlInput) els.aaPhotoboothReadyUrlInput.value = '';
                if (els.aaPhotoboothPublicUrlInput) els.aaPhotoboothPublicUrlInput.value = 'Memuat...';
                if (els.aaPhotoboothPublicUrlStatus) els.aaPhotoboothPublicUrlStatus.textContent = 'Memuat status public link...';
                if (els.aaPhotoboothDomainStatus) els.aaPhotoboothDomainStatus.textContent = 'Memuat status...';
                if (els.aaPhotoboothCustomDomainInput) els.aaPhotoboothCustomDomainInput.value = '';
                aaSetPhotoboothDomainMode('adaacara');
                aaSetPhotoboothDomainLocked({ mode: 'adaacara', status: 'standard', domain: '' });
                aaSyncPhotoboothLinkFields({ mode: 'adaacara', status: 'standard', domain: '', standard_url: '' });
                aaSyncPhotoboothPaymentPanel({ payment_status: 'not_required', can_upload_payment_proof: false });
                aaSetPhotoboothDomainMessage('', '');
                els.aaPhotoboothDomainModal.classList.add('is-open');
                try {
                    const payload = await aaRequestPhotoboothDomain(config.photoboothDomainUrl, { method: 'GET' });
                    aaApplyPhotoboothDomainPayload(payload);
                } catch (error) {
                    aaSetPhotoboothDomainMessage(error.message || 'Status domain belum bisa dimuat.', 'error');
                }
            }

            function aaClosePhotoboothDomainModal() {
                els.aaPhotoboothDomainModal?.classList.remove('is-open');
            }

            function aaBackToPublishChoiceFromPhotobooth() {
                aaClosePhotoboothDomainModal();
                aaShowPublishChoiceView();
                els.aaPublishModal?.classList.add('is-open');
            }

            function aaCopyPhotoboothStandardUrl() {
                if (!aaPhotoboothDomainStandardUrl) {
                    aaSetPhotoboothDomainMessage('Link Photobooth adaAcara belum tersedia.', 'error');
                    return;
                }
                const done = () => aaSetPhotoboothDomainMessage('Link Photobooth adaAcara tersalin.', 'success');
                if (navigator.clipboard && window.isSecureContext) {
                    navigator.clipboard.writeText(aaPhotoboothDomainStandardUrl).then(done).catch(done);
                    return;
                }
                const input = document.createElement('textarea');
                input.value = aaPhotoboothDomainStandardUrl;
            ...
            function aaCopyPhotoboothPublicUrl() {
                const value = aaGetPhotoboothPublicDomainUrl(aaPhotoboothDomainPayload);
                if (!value) {
                    aaSetPhotoboothDomainMessage('Public link custom domain belum tersedia.', 'error');
                    return;
                }
                const done = () => aaSetPhotoboothDomainMessage('Public link custom domain tersalin.', 'success');
                if (navigator.clipboard && window.isSecureContext) {
            ...
            async function aaPublishPhotoboothFrameFromModal() {
                storeCurrentPage();
                if (typeof aaValidatePhotoboothFramesForPublish === 'function') {
                    aaValidatePhotoboothFramesForPublish();
                }
                await publish();
            }

            els.aaPhotoboothDomainForm?.addEventListener('submit', async event => {
                event.preventDefault();
                if (!config.photoboothDomainUrl) return;
                const locked = aaIsPhotoboothDomainLocked(aaPhotoboothDomainPayload);
                if (aaPhotoboothPublishIntent && !aaCanPublishPhotoboothDomain(aaPhotoboothDomainPayload)) {
                    aaSetPhotoboothDomainMessage('Frame Photobooth custom domain bisa dipublish setelah domain aktif. Link adaAcara tetap bisa dipakai.', 'error');
                    showEditorToast('Custom domain belum aktif. Link adaAcara tetap bisa dipakai.', 'error');
                    return;
                }
                const button = els.aaPhotoboothDomainSubmitBtn;
                setButtonLoading(button, true, aaPhotoboothPublishIntent ? 'Publishing...' : 'Menyimpan...');
                aaSetPhotoboothDomainMessage(aaPhotoboothPublishIntent ? 'Menyiapkan publish frame Photobooth...' : 'Menyimpan pilihan domain...', '');
                if (aaPhotoboothPublishIntent) {
                    showEditorToast('Sedang menyimpan pilihan domain dan menerbitkan frame Photobooth...', 'saving', 'Publishing');
                }
                try {
                    let payload = aaPhotoboothDomainPayload;
                    if (!locked) {
                        const body = new URLSearchParams(new FormData(els.aaPhotoboothDomainForm));
                        if (config.csrfName && config.csrfHash) {
                            body.set(config.csrfName, config.csrfHash);
                        }
                        payload = await aaRequestPhotoboothDomain(config.photoboothDomainUrl, {
                            method: 'POST',
                            headers: {
                    ...
                    if (!aaPhotoboothPublishIntent) {
                        aaSetPhotoboothDomainMessage(payload?.message || 'Pilihan domain tersimpan.', 'success');
                        return;
                    }
                    aaSetPhotoboothDomainMessage('Sedang menerbitkan frame Photobooth...', '');
                    await aaPublishPhotoboothFrameFromModal();
                    setStatus('Frame Photobooth terpublish ke Memories');
                    aaSetPhotoboothDomainMessage('Frame Photobooth sudah terpublish.', 'success');
                    showEditorToast('Frame Photobooth sudah terpublish ke halaman Memories.', 'success');
                    aaClosePhotoboothDomainModal();
                    els.aaPublishModal?.classList.remove('is-open');
                } catch (error) {
                    const fallback = aaPhotoboothPublishIntent ? 'Frame Photobooth gagal dipublish.' : 'Pilihan domain belum bisa disimpan.';
                    setStatus(error.message || fallback, 'error');
                    aaSetPhotoboothDomainMessage(error.message || fallback, 'error');
                    if (aaPhotoboothPublishIntent) {
                        showEditorToast(error.message || fallback, 'error');
                    }
                } finally {
                    setButtonLoading(button, false);
                    if (els.aaEditorToast?.classList.contains('is-saving')) hideEditorToast();
            ...
            function aaOpenPhotoboothQrFromDomainModal() {
                const slug = normalizeSlug(els.aaPublishSlugInput?.value || config.initialSlug || config.initialTitle);
                if (!slug) {
                    aaSetPhotoboothDomainMessage('Isi slug URL dulu agar QR Photobooth bisa dibuka.', 'error');
                    showEditorToast('Isi slug URL dulu agar QR Photobooth bisa dibuka.', 'error');
                    return;
                }
                window.open(`${config.publicBaseUrl}${slug}/memories/qr`, '_blank', 'noopener');
            }

            els.aaPhotoboothPreviewUrlCopyBtn?.addEventListener('click', aaCopyPhotoboothStandardUrl);
            els.aaPhotoboothPublicUrlCopyBtn?.addEventListener('click', aaCopyPhotoboothPublicUrl);
            els.aaPhotoboothDomainQrBtn?.addEventListener('click', aaOpenPhotoboothQrFromDomainModal);
            els.aaPhotoboothDomainBackBtn?.addEventListener('click', aaBackToPublishChoiceFromPhotobooth);
            els.aaPhotoboothPaymentUploadBtn?.addEventListener('click', async () => {
                if (!config.photoboothDomainProofUrl) return;
                const file = els.aaPhotoboothPaymentProofInput?.files?.[0] || null;
                if (!file) {
                    if (els.aaPhotoboothPaymentProofStatus) {
                        els.aaPhotoboothPaymentProofStatus.textContent = 'Pilih file bukti pembayaran terlebih dahulu.';
                    }
                    return;
                }
                ...
                formData.set('payment_note', els.aaPhotoboothPaymentNoteInput?.value || '');
                if (config.csrfName && config.csrfHash) {
                    formData.set(config.csrfName, config.csrfHash);
                }

                setButtonLoading(els.aaPhotoboothPaymentUploadBtn, true, 'Mengupload...');
                if (els.aaPhotoboothPaymentProofStatus) {
                    els.aaPhotoboothPaymentProofStatus.textContent = 'Mengupload bukti pembayaran...';
                }
                try {
                    const payload = await aaRequestPhotoboothDomain(config.photoboothDomainProofUrl, {
                        method: 'POST',
                        body: formData,
                    });
                    aaApplyPhotoboothDomainPayload(payload);
                    aaSetPhotoboothDomainMessage(payload.message || 'Bukti pembayaran terkirim.', 'success');
                    showEditorToast(payload.message || 'Bukti pembayaran terkirim.', 'success');
                } catch (error) {
                    if (els.aaPhotoboothPaymentProofStatus) {
                        els.aaPhotoboothPaymentProofStatus.textContent = error.message || 'Bukti pembayaran gagal diupload.';
                    }
                    aaSetPhotoboothDomainMessage(error.message || 'Bukti pembayaran gagal diupload.', 'error');
                } finally {
                    setButtonLoading(els.aaPhotoboothPaymentUploadBtn, false);
                }
            });
            document.querySelectorAll('[data-aa-photobooth-domain-close]').forEach(button => {
                button.addEventListener('click', aaClosePhotoboothDomainModal);
            });
            els.aaPhotoboothDomainModal?.addEventListener('click', event => {
                if (event.target === els.aaPhotoboothDomainModal) aaClosePhotoboothDomainModal();
            });
            ...
            async function aaOpenPublishPhotoboothFlow() {
                if (config.canUseGuestMemories !== true) {
                    const isPurePhotoboothProject = normalizeProjectIntent(config.projectType || state.projectIntent || '') === 'photobooth';
                    openEditorAccessModal({
                        title: 'Aktifkan Photobooth Plus',
                        description: isPurePhotoboothProject ?
                            'Project ini khusus Photobooth. Aktifkan paket Plus dan minta admin mengaktifkan Guest Memories agar frame dan QR Photobooth bisa dipublish.' :
                            'Publish Frame Photobooth dan QR Photobooth tersedia untuk akun Plus. Kamu tetap bisa publish Undangan Digital dari tombol Publish Undangan.',
                        actionLabel: 'Lihat Paket Plus',
                    });
                    return;
                }
                try {
                    ...
                    els.aaPublishModal?.classList.remove('is-open');
                    await aaOpenPhotoboothDomainModal({ publishIntent: true });
                } catch (error) {
                    setStatus(error.message || 'Frame Photobooth gagal dipublish.', 'error');
                    showEditorToast(error.message || 'Frame Photobooth gagal dipublish.', 'error');
                }
            }

            els.aaPublishPhotoboothBtn?.addEventListener('click', aaOpenPublishPhotoboothFlow);
            els.aaPublishChoicePhotoboothBtn?.addEventListener('click', aaOpenPublishPhotoboothFlow);
            els.aaOpenPhotoboothQrBtn?.addEventListener('click', event => {
                if (config.canUseGuestMemories !== true) {
                    event.preventDefault();
                    openEditorAccessModal({
                        title: 'QR Photobooth Plus',
                        description: 'QR Photobooth tersedia setelah fitur Photobooth aktif di akun Plus. Publish Undangan Digital tetap bisa digunakan sesuai limit akun free.',
                        actionLabel: 'Lihat Paket Plus',
                    });
                    return;
                }
                ...
                els.aaOpenPhotoboothQrBtn.href = `${config.publicBaseUrl}${slug}/memories/qr`;
            });

            const aaCopyPublishPublicUrl = async () => {
                const value = String(state.publicUrl || els.aaPublicUrlInput?.value || '').trim();
                if (!value) {
                    showEditorToast('Link undangan belum tersedia.', 'error');
                    return;
                }
                try {
                    if (navigator.clipboard && window.isSecureContext) {
                        await navigator.clipboard.writeText(value);
                    } else if (els.aaPublicUrlInput) {
                        els.aaPublicUrlInput.focus();
                        els.aaPublicUrlInput.select();
                        document.execCommand('copy');
                        els.aaPublicUrlInput.blur();
                    }
                } catch (error) {
                    if (els.aaPublicUrlInput) {
                        els.aaPublicUrlInput.focus();
                        els.aaPublicUrlInput.select();
                    }
                    throw error;
                ...
                showEditorToast('Link undangan berhasil disalin.');
            };

            const aaCopyPublishSubdomainUrl = async () => {
                const value = String(els.aaPublishSubdomainUrlInput?.value || '').trim();
                if (!value) {
                    showEditorToast('Link subdomain belum tersedia.', 'error');
                    return;
                }
                try {
                    if (navigator.clipboard && window.isSecureContext) {
                        await navigator.clipboard.writeText(value);
                    } else if (els.aaPublishSubdomainUrlInput) {
                        els.aaPublishSubdomainUrlInput.focus();
                        els.aaPublishSubdomainUrlInput.select();
                        document.execCommand('copy');
                        els.aaPublishSubdomainUrlInput.blur();
                    }
                } catch (error) {
                ...
                const domainStatus = String(config.publishedDomain?.status || '');
                setStatus('Link subdomain disalin');
                showEditorToast(domainStatus === 'active'
                    ? 'Link subdomain aktif berhasil disalin.'
                    : 'Link subdomain berhasil disalin. Alamat ini bisa digunakan setelah aktivasi selesai.');
            };

            els.aaPublicUrlCopyBtn?.addEventListener('click', async () => {
                try {
                    await aaCopyPublishPublicUrl();
                } catch (error) {
                    showEditorToast('Link belum bisa disalin otomatis. Silakan salin manual dari kolom Public link.', 'error');
                }
            });
            els.aaPhotoboothReadyUrlCopyBtn?.addEventListener('click', async () => {
                const value = String(els.aaPhotoboothReadyUrlInput?.value || '').trim();
                if (!value || value === 'Link undangan belum tersedia') {
                    showEditorToast('Link undangan belum tersedia.', 'error');
                    return;
                }
                try {
                    if (navigator.clipboard && window.isSecureContext) {
                        await navigator.clipboard.writeText(value);
                    } else if (els.aaPhotoboothReadyUrlInput) {
                        els.aaPhotoboothReadyUrlInput.focus();
                        els.aaPhotoboothReadyUrlInput.select();
                        document.execCommand('copy');
                        els.aaPhotoboothReadyUrlInput.blur();
                    }
                    setStatus('Link disalin');
                    showEditorToast('Link undangan berhasil disalin.');
                } catch (error) {
                    showEditorToast('Link undangan belum bisa disalin otomatis. Silakan salin manual dari kolom Public Link.', 'error');
                }
            });
            ...
            els.aaPublishSubdomainCopyBtn?.addEventListener('click', async () => {
                try {
                    await aaCopyPublishSubdomainUrl();
                } catch (error) {
                    showEditorToast('Link subdomain belum bisa disalin otomatis. Silakan salin manual dari kolom subdomain.', 'error');
                }
            });
            els.aaPublishSaveDraftBtn?.addEventListener('click', async () => {
                const button = els.aaPublishSaveDraftBtn;
                setButtonLoading(button, true, 'Menyimpan...');
                try {
                    await saveDraft(false);
                } catch (error) {
            ...
            els.aaClosePublishSecondaryBtn?.addEventListener('click', () => {
                els.aaPublishModal?.classList.remove('is-open');
            });
            ...
            function aaCloseCanvasDrawerForObjectSelection() {
	            ...
	            state.canvas.on('selection:created', function(event) {
	                ...
	                aaApplyTextSelectionVisualPadding?.();
	                syncInspector();
                ...
                aaSetStaggerControlVisible(aaIsTextAnimationTarget(state.canvas.getActiveObject()));
	                ...
	                aaRenderRecentFonts();
                    aaScheduleLayerPanelRender();
	                if (typeof scheduleMobileInteractionDrawerSync === 'function') scheduleMobileInteractionDrawerSync();
	                requestAnimationFrame(syncObjectFloatingToolbar);
	            });
            ...
            state.canvas.on('selection:cleared', () => {
                if (state.cropPanTarget && typeof finishImageCropPanMode === 'function') {
                    const cropPanTarget = state.cropPanTarget;
                    const objects = state.canvas?.getObjects?.() || [];
                    if (cropPanTarget && objects.includes(cropPanTarget)) {
                        state.canvas.setActiveObject(cropPanTarget);
                        if (typeof aaSyncImageCropPanUi === 'function') {
	            ...
	            state.canvas.on('object:modified', function(event) {
	                ...
	                aaApplyTextSelectionVisualPadding?.(event.target || state.canvas.getActiveObject());
	            ...
	            state.canvas.on('object:moving', () => {
	                aaBeginSelectionTransformUi();
	                const isMobileNativeDrag = window.matchMedia('(max-width: 767px)').matches &&
	                    Boolean(state.canvas?._currentTransform?.target);
	                if (!isMobileNativeDrag) {
	                    syncObjectFloatingToolbar();
	                    syncCountdownContextToolbar();
	                    syncInteractionPopover();
	                }
		            ...
		            state.canvas.on('object:scaling', function(event) {
		                ...
		                const isMobileNativeTransform = window.matchMedia('(max-width: 767px)').matches &&
		                    Boolean(state.canvas?._currentTransform?.target);
		                if (!isMobileNativeTransform) {
		                    syncObjectFloatingToolbar();
		                    syncCountdownContextToolbar();
		                    syncInteractionPopover();
		                }
            ...
            state.canvas.on('text:editing:entered', event => {
                const isMobileTextEditing = window.matchMedia('(max-width: 767px)').matches;
                document.body.classList.toggle('aa-mobile-text-editing', isMobileTextEditing);
                document.body.classList.remove('aa-mobile-tools-open', 'aa-mobile-share-open');
                aaApplyTextSelectionVisualPadding?.(event.target);
                if (isMobileTextEditing) {
                    if (typeof isLinkInteractionObject === 'function' && typeof isCopyInteractionObject === 'function' &&
                        (isLinkInteractionObject(event.target) || isCopyInteractionObject(event.target))) {
                        event.target?.exitEditing?.();
                        document.body.classList.remove('aa-mobile-text-editing', 'aa-mobile-properties-open');
                        syncInteractionPopover?.(event.target);
                        state.canvas.requestRenderAll();
                        return;
                    }
                    event.target?.exitEditing?.();
                    window.aaOpenMobileTextProperties?.({ focus: false });
                    state.canvas.requestRenderAll();
                }
            });
            state.canvas.on('text:editing:exited', () => {
                window.setTimeout(function() {
                    document.body.classList.remove('aa-mobile-text-editing');
                }, 180);
            });
            state.canvas.on('mouse:dblclick', function(event) {
                if (typeof startImageCropPanMode === 'function') {
                    startImageCropPanMode(event);
                }
            });
            state.canvas.on('mouse:down', function(event) {
                if (typeof startImageCropPanDrag === 'function' && startImageCropPanDrag(event)) {
                    return;
                }
                const sourceEvent = event?.e || null;
            ...
            state.canvas.on('mouse:move', function(event) {
                if (typeof moveImageCropPanDrag === 'function') {
                    moveImageCropPanDrag(event);
                }
            });
            state.canvas.on('mouse:up', () => {
                if (typeof finishImageCropPanDrag === 'function') {
                    finishImageCropPanDrag();
                }
                aaEndSelectionTransformUi();
            ...
            function aaIsTransientCanvasObject(object) {
                if (!object) return false;
                const customType = String(object.customType || '');
                return object.excludeFromExport === true ||
                    customType === 'crop-pan-ghost' ||
                    customType === 'crop-helper' ||
                    customType === 'selection-helper';
            }

            function aaScheduleHistorySnapshot(event = null) {
                if (aaIsTransientCanvasObject(event?.target)) {
                    return;
                }
                if (
                    state.isRestoring ||
            ...
            state.canvas.on('object:removed', function(event) {
                if (aaIsTransientCanvasObject(event?.target)) {
                    return;
                }
                aaScheduleLayerPanelRender();
            ...
            document.addEventListener('keydown', event => {
                ...
                const isTyping = ['input', 'textarea', 'select'].includes(activeTag) || document
	                ...
	                if (activeObject?.isEditing === true) return;
            ...
            window.addEventListener('resize', () => {
                ...
                positionCountdownDatePicker();
	                const isMobileTextEditing = document.body.classList.contains('aa-mobile-text-editing') ||
	                    (window.matchMedia('(max-width: 767px)').matches && state.canvas?.getActiveObject?.()?.isEditing === true);
	                if (!isMobileTextEditing) {
	                    fitZoomUnlessManual();
	                }
                syncObjectFloatingToolbar();
        ...
        function configureEditorFabricSelectionControls() {
            if (!window.fabric || fabric.Object.prototype.__aaEditorSelectionControlsConfigured) return;

            const primary = '#7c3aed';
            const primaryDark = '#6d28d9';
            const activeFill = '#f3e8ff';
            const isMobileControls = window.matchMedia('(max-width: 767px)').matches;
            const handleSize = isMobileControls ? 26 : 22;
            const sideHandleLong = isMobileControls ? 36 : 30;
            const sideHandleShort = isMobileControls ? 16 : 12;
            const rotateSize = isMobileControls ? 74 : 58;

            function aaActiveTransformCorner(fabricObject = null) {
                const transform = state?.canvas?._currentTransform || null;
                if (!transform || (fabricObject && transform.target !== fabricObject)) return '';
                return transform.corner || '';
            }

            function aaIsOutsideSelectionTransformingObject(fabricObject = null) {
            ...
            function aaShouldHideSelectionControl(name, fabricObject = null) {
                if (aaIsOutsideSelectionTransformingObject(fabricObject)) return true;
                const activeCorner = aaActiveTransformCorner(fabricObject);
                return Boolean(activeCorner && activeCorner !== name);
            }

            function aaIsSelectionControlActive(name, fabricObject = null) {
                return aaActiveTransformCorner(fabricObject) === name || fabricObject?.__corner === name;
            }

            function renderCircleControl(name, size, fill = '#ffffff', stroke = primary, lineWidth = 2.5) {
                return function(ctx, left, top, styleOverride, fabricObject) {
                    if (aaShouldHideSelectionControl(name, fabricObject)) return;

                    const isActive = aaIsSelectionControlActive(name, fabricObject);
                    ...
                    ctx.strokeStyle = isActive ? primaryDark : stroke;
                    ctx.stroke();
                    ctx.lineWidth = 1;
            ...
            function renderPillControl(name, width, height) {
                return function(ctx, left, top, styleOverride, fabricObject) {
                    if (aaShouldHideSelectionControl(name, fabricObject)) return;
                    ...
                    roundedRectPath(ctx, width, height, Math.min(width, height) / 2);
                    ...
                    ctx.strokeStyle = isActive ? primaryDark : primary;
                    ctx.stroke();
                    ctx.restore();
                };
            }

            function drawRotateArrow(ctx, radius, start, end) {
                const arrowSize = 6.4;
                ctx.beginPath();
                ...
                ctx.moveTo(x, y);
                ctx.lineTo(
                    x - Math.cos(tangent - 0.72) * arrowSize,
                    y - Math.sin(tangent - 0.72) * arrowSize
                );
                ctx.moveTo(x, y);
                ctx.lineTo(
                    x - Math.cos(tangent + 0.72) * arrowSize,
            ...
            function renderRotateControl(ctx, left, top, styleOverride, fabricObject) {
                if (aaShouldHideSelectionControl('mtr', fabricObject)) return;

                const isActive = aaIsSelectionControlActive('mtr', fabricObject);

                ctx.save();
                ctx.translate(left, top);
                ctx.shadowColor = isActive ? 'rgba(124, 58, 237, .42)' : 'rgba(124, 58, 237, .28)';
                ctx.shadowBlur = isActive ? 28 : 24;
                ctx.shadowOffsetY = isActive ? 12 : 11;
                ctx.beginPath();
                ctx.arc(0, 0, rotateSize / 2, 0, Math.PI * 2);
                ctx.fillStyle = isActive ? activeFill : '#ffffff';
                ctx.fill();
                ctx.shadowColor = 'transparent';
                ctx.shadowBlur = 0;
                ctx.shadowOffsetY = 0;
                ctx.lineWidth = isActive ? 3.8 : 3.2;
                ctx.strokeStyle = isActive ? primaryDark : primary;
                ctx.stroke();
                ctx.strokeStyle = isActive ? primaryDark : primary;
                ctx.lineWidth = 3.2;
                ctx.lineCap = 'round';
                ctx.lineJoin = 'round';
                drawRotateArrow(ctx, 12.5, 1.12 * Math.PI, 1.92 * Math.PI);
                drawRotateArrow(ctx, 12.5, 0.12 * Math.PI, 0.92 * Math.PI);
                ctx.restore();
            }

            fabric.Object.prototype.transparentCorners = false;
            fabric.Object.prototype.cornerColor = '#ffffff';
            fabric.Object.prototype.cornerStrokeColor = primary;
            fabric.Object.prototype.borderColor = primary;
            fabric.Object.prototype.borderScaleFactor = 4.2;
            ...
            ['tl', 'tr', 'br', 'bl'].forEach(function(name) {
                if (controls[name]) {
                    controls[name].render = renderCircleControl(name, handleSize);
                }
            });
            ['mt', 'mb'].forEach(function(name) {
                if (controls[name]) {
                    controls[name].sizeX = sideHandleLong + 8;
                    controls[name].sizeY = sideHandleShort + 10;
                    controls[name].render = renderPillControl(name, sideHandleLong, sideHandleShort);
                }
            });
            ['ml', 'mr'].forEach(function(name) {
                if (controls[name]) {
                    controls[name].sizeX = sideHandleShort + 10;
                    controls[name].sizeY = sideHandleLong + 8;
                    controls[name].render = renderPillControl(name, sideHandleShort, sideHandleLong);
                }
            });

            if (controls.mtr) {
                controls.mtr.y = 0.5;
                controls.mtr.offsetY = isMobileControls ? 88 : 64;
                controls.mtr.sizeX = rotateSize;
                controls.mtr.sizeY = rotateSize;
                controls.mtr.touchSizeX = rotateSize + (isMobileControls ? 22 : 10);
                controls.mtr.touchSizeY = rotateSize + (isMobileControls ? 22 : 10);
                controls.mtr.withConnection = false;
                controls.mtr.cursorStyle = 'grab';
                controls.mtr.render = renderRotateControl;
            }

            if (fabric.IText) {
                fabric.IText.prototype.editingBorderColor = primary;
            }
            if (fabric.Textbox) {
                fabric.Textbox.prototype.editingBorderColor = primary;
            }

            fabric.Object.prototype.__aaEditorSelectionControlsConfigured = true;
        }

        function installTabletOrientationGuard() {
            if (state.__aaTabletOrientationGuardInstalled) return;
            state.__aaTabletOrientationGuardInstalled = true;

            const modal = document.getElementById('aaTabletLandscapeModal');
            let timer = null;

            function syncTabletOrientationGuard() {
                const isTabletViewport = window.matchMedia('(min-width: 768px) and (max-width: 1366px)').matches;
                const isCoarsePointer = window.matchMedia('(pointer: coarse)').matches;
                const isTabletMode = isTabletViewport && isCoarsePointer;
                const isPortrait = window.innerHeight > window.innerWidth;
                const shouldBlock = isTabletMode && isPortrait;

                document.body.classList.toggle('aa-editor-tablet-mode', isTabletMode);
                document.body.classList.toggle('aa-editor-orientation-blocked', shouldBlock);
                modal?.classList.toggle('is-visible', shouldBlock);
            }

            function scheduleSync() {
                clearTimeout(timer);
                timer = window.setTimeout(syncTabletOrientationGuard, 140);
            }

            syncTabletOrientationGuard();
            window.addEventListener('resize', scheduleSync, { passive: true });
            window.addEventListener('orientationchange', scheduleSync, { passive: true });
            window.screen?.orientation?.addEventListener?.('change', scheduleSync);
        }

	        function installMobileEditorMode() {
	            if (state.__aaMobileEditorModeInstalled) return;
	            state.__aaMobileEditorModeInstalled = true;

            const toolsButton = document.getElementById('aaMobileToolsBtn');
            const shareButton = document.getElementById('aaMobileShareBtn');
            const textEditButton = document.getElementById('aaMobileTextEditBtn');
            const textDoneButton = document.getElementById('aaMobileTextDoneBtn');
            const textCloseButton = document.getElementById('aaMobileTextCloseBtn');
            const rightbar = document.querySelector('.aa-rightbar');
            const leftbar = document.querySelector('.aa-leftbar');
            const leftDrawer = document.querySelector('.aa-left-drawer');
            ...
            function isMobileViewport() {
	            ...
	            function syncMobileKeyboardOffset() {
	                if (!isMobileViewport()) {
	                    document.documentElement.style.removeProperty('--aa-mobile-keyboard-offset');
	                    document.documentElement.style.removeProperty('--aa-mobile-viewport-height');
	                    return;
                }
                const viewport = window.visualViewport || null;
                ...
                const keyboardOffset = Math.max(0, Math.round(layoutHeight - viewportHeight - offsetTop));
	                document.documentElement.style.setProperty('--aa-mobile-keyboard-offset', `${keyboardOffset}px`);
	                document.documentElement.style.setProperty('--aa-mobile-viewport-height', `${Math.round(viewportHeight)}px`);
	            }

	            function recoverEditorViewportAfterResume() {
	                if (!state.canvas) return;
            ...
            function closeMobileTextProperties() {
                document.body.classList.remove('aa-mobile-properties-open', 'aa-mobile-text-editing');
                const active = state.canvas?.getActiveObject?.();
                if (active?.isEditing) {
                    active.exitEditing?.();
                    state.canvas.requestRenderAll();
            ...
            function openMobileTextProperties(options = {}) {
                if (!isMobileViewport()) return false;
                const active = state.canvas?.getActiveObject?.();
                const textTarget = typeof getContextTextTarget === 'function' ? getContextTextTarget(active) : active;
                if (!textTarget || !els.aaTextInput || els.aaTextInput.disabled) return false;
                if (active?.isEditing) {
                    active.exitEditing?.();
                    state.canvas.requestRenderAll();
                }
                document.body.classList.add('aa-mobile-properties-open');
                document.body.classList.remove('aa-mobile-tools-open', 'aa-mobile-share-open');
                if (typeof hideObjectFloatingToolbar === 'function') hideObjectFloatingToolbar();
                if (typeof hideCountdownContextToolbar === 'function') hideCountdownContextToolbar();
                if (typeof hideInteractionPopover === 'function') hideInteractionPopover();
                if (typeof syncInspector === 'function') syncInspector();
                if (options.focus === true) {
                    window.setTimeout(() => els.aaTextInput?.focus({ preventScroll: true }), 90);
                }
                syncMobileKeyboardOffset();
                return true;
            }

            function syncMobileDrawerLayer() {
                const drawerOpen = isMobileViewport() && Boolean(
                    leftbar?.classList.contains('is-drawer-open') ||
                    leftDrawer?.classList.contains('is-pinned')
                );
                document.body.classList.toggle('aa-mobile-drawer-active', drawerOpen);
            }

            function setMobilePanel(panel) {
                const nextPanel = panel || '';
                document.body.classList.toggle('aa-mobile-tools-open', nextPanel === 'tools');
                document.body.classList.toggle('aa-mobile-share-open', nextPanel === 'share');
                if (nextPanel) {
                    closeMobileTextProperties();
                }
                toolsButton?.setAttribute('aria-expanded', nextPanel === 'tools' ? 'true' : 'false');
                shareButton?.setAttribute('aria-expanded', nextPanel === 'share' ? 'true' : 'false');
            }

            let timer = null;
            function syncMobileEditorMode() {
                ...
                if (!isMobileViewport) {
                    setMobilePanel('');
                    closeMobileTextProperties();
                    document.body.classList.remove('aa-mobile-drawer-active');
                }
                syncMobileDrawerLayer();
                syncMobileKeyboardOffset();
            }

            function scheduleSync() {
                clearTimeout(timer);
                timer = window.setTimeout(syncMobileEditorMode, 120);
            }

            syncMobileEditorMode();
	            window.addEventListener('resize', scheduleSync, { passive: true });
	            window.addEventListener('orientationchange', scheduleSync, { passive: true });
	            window.screen?.orientation?.addEventListener?.('change', scheduleSync);
	            window.visualViewport?.addEventListener?.('resize', syncMobileKeyboardOffset, { passive: true });
	            window.visualViewport?.addEventListener?.('scroll', syncMobileKeyboardOffset, { passive: true });
	            window.addEventListener('pageshow', recoverEditorViewportAfterResume, { passive: true });
	            window.addEventListener('focus', recoverEditorViewportAfterResume, { passive: true });
	            document.addEventListener('visibilitychange', function() {
	                if (document.visibilityState === 'visible') {
	                    recoverEditorViewportAfterResume();
	                }
	            });
            toolsButton?.addEventListener('click', function(event) {
                event.preventDefault();
                event.stopPropagation();
                setMobilePanel(document.body.classList.contains('aa-mobile-tools-open') ? '' : 'tools');
            });
            shareButton?.addEventListener('click', function(event) {
                event.preventDefault();
                event.stopPropagation();
                setMobilePanel(document.body.classList.contains('aa-mobile-share-open') ? '' : 'share');
            });
            textEditButton?.addEventListener('click', function(event) {
                event.preventDefault();
                event.stopPropagation();
                openMobileTextProperties({ focus: true });
            });
            textDoneButton?.addEventListener('click', function(event) {
                event.preventDefault();
                event.stopPropagation();
                closeMobileTextProperties();
                state.canvas?.requestRenderAll?.();
                if (typeof storeCurrentPage === 'function') storeCurrentPage();
            ...
            rightbar?.addEventListener('pointerdown', event => event.stopPropagation());
            rightbar?.addEventListener('click', event => event.stopPropagation());
            document.addEventListener('click', function(event) {
                if (!document.body.classList.contains('aa-editor-mobile-mode')) return;
                const target = event.target;
                if (target?.closest?.('#aaMobileToolsBtn, #aaMobileShareBtn, #aaMobileTextEditBtn, .aa-left-rail, .aa-tablet-action-dock, .aa-rightbar')) {
                    return;
                }
                setMobilePanel('');
            });
            [leftbar, leftDrawer].filter(Boolean).forEach(element => {
                new MutationObserver(syncMobileDrawerLayer).observe(element, {
                    attributes: true,
                    attributeFilter: ['class'],
                });
            });
            syncMobileDrawerLayer();
	            window.aaOpenMobileTextProperties = openMobileTextProperties;
	            window.aaCloseMobileTextProperties = closeMobileTextProperties;
	        }

	        function installEditorBrowserZoomGuard() {
	            if (state.__aaEditorBrowserZoomGuardInstalled) return;
	            state.__aaEditorBrowserZoomGuardInstalled = true;

	            document.body.classList.add('aa-editor-browser-zoom-locked');

	            function isEditorGestureTarget(target) {
	                return Boolean(target?.closest?.('.aa-studio-shell, .aa-desktop-only-modal'));
	            }

	            function preventEditorGesture(event) {
	                if (!isEditorGestureTarget(event.target)) return;
	                event.preventDefault();
	            }

	            ['gesturestart', 'gesturechange', 'gestureend'].forEach(function(eventName) {
	                document.addEventListener(eventName, preventEditorGesture, { passive: false });
	            });
	        ...
	        async function init() {
	            ...
	            installEditorBrowserZoomGuard();
	            installMobileEditorMode();
	            installTabletOrientationGuard();
            ...
            configureEditorFabricSelectionControls();
            ...
            configureEditorFabricSelectionControls();
            if (typeof installIntegratedCropControls === 'function') {
                installIntegratedCropControls();
            }

            bindObjectContextMenu();
            bindEvents();
            bindPhotoboothEntryToast();
...
function aaRestoreImageFrameState(image, frameState) {
    ...
    syncImageFrameButtons?.(image);
            ...
            function getLayerVisualScore(item, activeRect) {
                if (!item || !item.object || !activeRect) return 0;

                const targetRect = getRect(item.object);
                if (!targetRect) return 0;

                const activeArea = Math.max(1, (Number(activeRect.width) || 0) * (Number(activeRect.height) || 0));
                const targetArea = Math.max(1, (Number(targetRect.width) || 0) * (Number(targetRect.height) || 0));
                const overlapArea = Math.max(0, Number(item.overlapArea) || 0);
                const overlapRatio = overlapArea / Math.min(activeArea, targetArea);

                // Abaikan sentuhan kecil supaya layer tidak lompat karena overlap tipis.
                if (overlapArea < 36 || overlapRatio < 0.04) return 0;

                let typeWeight = 1;
                if (item.object.type === 'image') typeWeight = 1.55;
                if (item.object.type === 'group') typeWeight = 1.22;
                if (item.object.customType === 'photo-gallery') typeWeight = 1.35;
                if (item.object.customType === 'frame-placeholder') typeWeight = 1.3;

                const sizeWeight = Math.min(3, Math.sqrt(targetArea / activeArea));
                return overlapArea * typeWeight * (1 + sizeWeight * 0.28);
            }

            function pickPrimaryVisualLayerTarget(items, activeRect, direction) {
                const candidates = (items || [])
                    .map(function(item) {
                        return {
                            ...
                            visualScore: getLayerVisualScore(item, activeRect)
                        };
                    })
                    .filter(function(item) {
                        return item.visualScore > 0;
                    });

                if (!candidates.length) return null;

                candidates.sort(function(a, b) {
                    if (Math.abs(b.visualScore - a.visualScore) > 0.01) {
                        return b.visualScore - a.visualScore;
                    }

                    return direction < 0 ? b.index - a.index : a.index - b.index;
                });
            ...
            function moveBackwardOneVisualLayer(canvas, active) {
                ...
                const activeRect = getRect(active);
                const belowCandidates = overlaps.filter(function(item) {
                    return item.index < activeIndex;
                });
                const primaryBelow = pickPrimaryVisualLayerTarget(belowCandidates, activeRect, -1);

                if (primaryBelow && primaryBelow.object) {
                    return moveBefore(objects, active, primaryBelow.object);
                }

                // Cari object overlap paling dekat di bawah active.
                const below = belowCandidates
                    .sort(function(a, b) {
            ...
            function moveForwardOneVisualLayer(canvas, active) {
                ...
                const overlaps = getOverlappingLayerObjects(canvas, active);
                const activeRect = getRect(active);
                const aboveCandidates = overlaps.filter(function(item) {
                    return item.index > activeIndex;
                });
                const primaryAbove = pickPrimaryVisualLayerTarget(aboveCandidates, activeRect, 1);

                if (primaryAbove && primaryAbove.object) {
                    return moveAfter(objects, active, primaryAbove.object);
                }

                // Cari object overlap paling dekat di atas active.
                const above = aboveCandidates
                    .sort(function(a, b) {
                        return a.index - b.index;
                    })[0];
