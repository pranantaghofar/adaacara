...
<head>
    ...
    <title>Tambah Template - Ada Acara</title>
...
<body class="aa-app-ui min-h-screen bg-[#eef8f5] text-slate-900 antialiased">
    <?= view('admin/partials/header', ['adminTitle' => 'Tambah Template', 'adminIcon' => 'template', 'adminActive' => 'templates']) ?>
    <main class="mx-auto max-w-[1700px] px-4 py-8 sm:px-6">
        ...
        <form class="grid gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" action="<?= site_url('admin/templates/store') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
